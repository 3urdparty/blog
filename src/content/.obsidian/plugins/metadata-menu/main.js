...
var BuiltinSummarizingFunctionDescription = {
	...
	Average:
		'Returns the average value of <{{summarizedFieldName}}> fields in the pages matching the query',
...
var BuiltinSummarizingFunction = {
	...
	Average:
		'const i=0.0;const sum = pages.reduce((p, c) => p + c["{{summarizedFieldName}}"], i); return sum / pages.length',
...
var Helper = {
	...
	BuiltinSummarizing: '',
	CustomList:
		'Javascript string, the "page" (dataview page type) variable is available\nexample 1: page.file.name\nexample 2: `${page.file.name} of gender ${page.gender}`',
	CustomBulletList:
		'Javascript string, the "page" (dataview page type) variable is available\nexample 1: page.file.name\nexample 2: `${page.file.name} of gender ${page.gender}`',
	CustomSummarizing:
		'Javascript string, the "pages" (dataview pages type) variable is available\nexample: \nconst initialValue = 0;\nconst sumWithInitial = pages.reduce(\n    (previousValue, currentValue) => previousValue + currentValue,\n    initialValue\n);\nreturn `${sumWithInitial}`\n'
}
var Default = {
	LinksList: '',
	LinksBulletList: '',
...
var genericFieldRegex =
	'(?<inQuote>>*(\\s+)?)?(?<inList>- )?(?<preSpacer>(\\s+)?)?(?<startStyle>[_\\*~`]*)(?<attribute>[-\\w\\p{Letter}\\p{Emoji_Presentation}\\s]*)(?<endStyle>[_\\*~`]*)(?<beforeSeparatorSpacer>\\s*)'
var fullLineRegex = new RegExp(
	`^\\s*${genericFieldRegex}::(?<afterSeparatorSpacer>\\s*)(?<values>.*)?`,
	'u'
)
var inSentenceRegexBrackets = new RegExp(
	`\\[${genericFieldRegex}::(?<afterSeparatorSpacer>\\s*)(?<values>[^\\]]+)?\\]`,
	'gu'
)
...
var getLink = (linkText, source) => {
	...
	if ((_b = fR == null ? void 0 : fR.groups) == null ? void 0 : _b.target) {
		const path =
			(_e = app.metadataCache.getFirstLinkpathDest(
				(_c = fR == null ? void 0 : fR.groups) == null ? void 0 : _c.target,
				(source == null ? void 0 : source.path) ||
					((_d = fR == null ? void 0 : fR.groups) == null ? void 0 : _d.target)
			)) == null
				? void 0
				: _e.path
		if (path) {
...
var extractLinks = (rawContent) => {
	...
	while (
		(rawLink =
			(_b = (_a = linksIterator.next()) == null ? void 0 : _a.value) == null ? void 0 : _b[0])
	) {
		links.push(rawLink)
	}
	return links
...
var frontMatterLineField = (line) => {
	const frontMatterRegex = new RegExp(
		/(?<indentation>\s*)(?<list>-\s)?(?<attribute>[-\w\p{Letter}\p{Emoji_Presentation}\s\(\)]*[^\s])(?<beforeSeparatorSpacer>\s*):(?<afterSeparatorSpacer>\s*)(?<values>.*)/u
	)
	...
	if (fR == null ? void 0 : fR.groups) {
		...
		while (!next.done) {
			if (next.value.groups) {
				...
				fields.push({
					attribute,
					values: decodeLink(values),
					index: next.value.index || 0,
					length: next.value[0].length,
					inList,
					inQuote,
					preSpacer,
					startStyle,
...
var fieldDecorations = ['Code', 'Italic', 'Bold', 'Strikethrough']
var FieldHTMLTagMap = {
	Code: 'pre',
	Italic: 'i',
	Bold: 'b',
...
var createGenerator =
	(generateForCustomCharacters2, specialRandomBytes2, randomBytes) =>
...
async function postValues(
	...
	for (const item of payload) {
		...
		const field2 =
			(_a = plugin.fieldIndex.filesFields.get(file.path)) == null
				? void 0
				: _a.find((f) => f.id === id)
...
var BaseValueModal = class extends import_obsidian3.Modal {
	...
	async onClose() {
		var _a
		if (!this.saved) (_a = this.managedField.previousModal) == null ? void 0 : _a.open()
		this.saved = false
	}
	async save() {
		this.saved = true
...
var MultiTargetModificationConfirmModal = class extends import_obsidian3.Modal {
	...
	async onOpen() {
		...
		for (const file of this.managedField.target) {
			const eF = await getExistingFieldForIndexedPath(
				this.managedField.plugin,
				file,
				this.managedField.indexedPath || this.managedField.id
			)
			targets.push({
				filePath: file.path,
		...
		for (const target of targets) {
			...
			const positionIconContainer = locationContainer.createSpan({
				cls: 'position-icon',
			...
			const lineNumberDisplay = locationContainer.createSpan({
				cls: 'line-number',
				text: lineNumber ? `(${lineNumber})` : ''
			})
			if (lineNumber)
				lineNumberDisplay.ariaLabel = `${this.managedField.name} current line: ${lineNumber}`
			const currentValueContainer = targetContainer.createDiv({
				cls: 'current-value',
				text: getValueDisplay(target.existingField)
			})
		...
		new import_obsidian3.ButtonComponent(footer)
					...
					if (next.value.groups) {
						...
						if (optionsString) {
							try {
								...
								this.buildTemplateSelectItem(
									options2.template,
									this.contentEl.createDiv({ cls: 'field-container' }),
									name,
									selectOptions
								)
...
in <${this.managedField.name}> ${this.managedField.fileClassName ? this.managedField.fileClassName : 'Metadata Menu'} settings`
								if (errorName === 'SyntaxError') new import_obsidian4.Notice(notice, 5e3)
							}
						} else {
							this.buildTemplateInputItem(
								options2.template,
								this.contentEl.createDiv({ cls: 'field-container' }),
								name
							)
...
function createDvField(managedField, dv, p, fieldContainer, attrs = {}) {
	...
	if ((_a = attrs.options) == null ? void 0 : _a.alwaysOn) spacer.hide()
	...
	if (!((_b = attrs == null ? void 0 : attrs.options) == null ? void 0 : _b.alwaysOn)) {
		...
		fieldContainer.onmouseout = () => {
			var _a2
			editBtn.hide()
			if (!((_a2 = attrs.options) == null ? void 0 : _a2.alwaysOn)) spacer.show()
		}
	}
...
function actions(plugin, field2, file, location, indexedPath) {
	...
	const action = async () => {
		...
		;(_a = fieldValueManager(plugin, field2.id, field2.fileClassName, file, eF, indexedPath)) ==
		null
			? void 0
			: _a.openModal()
	}
	if (isSuggest(location)) {
		...
		location.addOption(
			`field_${field2.id}_update`,
			iconName,
			action,
			`Update ${field2.name}'s value`
		)
	}
}
...
async function enterFieldSetting(settingModal, field2, speed = 100) {
	...
	if (((_a = field2.options) == null ? void 0 : _a.template) !== void 0) {
		settingModal.plugin.testRunner.insertInTextComponent(
			settingModal.templateValue,
			field2.options.template
		)
	...
	return class ValueModal extends base {
		...
		async save() {
			if (!validateValue2(this.managedField)) {
				...
				this.errorField.setText(
					`value must be numeric${min2 ? ' and >= ' + min2 : ''} ${max2 ? ' and <= ' + max2 : ''}`
				)
...
function actions2(plugin, field2, file, location, indexedPath) {
	...
	const increase = async () => {
		const eF = await getExistingFieldForIndexedPath2(plugin, file, indexedPath)
		const fieldVM = fieldValueManager(
			plugin,
			field2.id,
	...
	if (isSuggest(location)) {
		...
		if (step) {
			location.addOption(
				`field_${field2.id}_decrease`,
				'minus-square',
				decrease,
				`Decrease ${name} by ${step}`
			)
			location.addOption(
				`field_${field2.id}_increase`,
				'plus-square',
				increase,
				`Increase ${name} by ${step}`
			)
		}
...
function getOptionsStr2(field2) {
	...
	Object.keys(field2.options).forEach((k) => {
		if (field2.options[k]) options2.push(`${k}: ${field2.options[k]}`)
	})
		...
		function runModifierEffects() {
			state.orderedModifiers.forEach(function (_ref3) {
				var name = _ref3.name,
					...
					options3 = _ref3$options === void 0 ? {} : _ref3$options,
...
function settingsModal3(Base25) {
	return class SettingsModal extends Base25 {
		...
		createSettingContainer() {
			...
			Object.keys(SourceType).forEach((option) =>
				this.sourceTypeSelector.addOption(option, SourceTypeDisplay[option])
			)
			...
			vPC.forEach((input, i) => {
				...
				if (vPC.length > 1) {
					...
					if (prevComponent.inputEl.value === input.inputEl.value) {
						setValidationError(input, "Two adjacent values can't be equal")
						error = true
					}
					prevComponent = input
				}
				if (
					/[,]/gu.test(input.inputEl.value) &&
					((_a = input.inputEl.parentElement) == null ? void 0 : _a.lastElementChild)
				) {
					setValidationError(input, 'Values cannot contain a comma')
					error = true
				}
				if (
					input.inputEl.value == '' &&
					((_b = input.inputEl.parentElement) == null ? void 0 : _b.lastElementChild)
				) {
					setValidationError(input, "Values can't be null.")
					error = true
				}
			})
			return !error
		...
		createListNotePathContainer(container) {
			...
			input.inputEl.addClass('full-width')
			input.inputEl.addClass('with-label')
			new FileSuggest(input.inputEl, this.plugin, '/')
			...
			if (key != Object.keys(this.field.options)[0]) {
				...
				valueUpgradeButton.onClick((evt) => {
					...
					const upperComponent =
						inputIndex !== -1
							? this.valuesPromptComponents[inputIndex - 1]
							: this.valuesPromptComponents.last()
					if (upperComponent) {
		...
		createValuesFromDVQueryContainer(parentContainer) {
			...
			valuesFromDVQueryTopContainer.createEl('span', {
				text: 'Dataview query returning a list of string (<dv> object is available)',
				cls: 'sub-text'
			})
			const valuesFromDVQueryContainer = valuesFromDVQueryTopContainer.createDiv({
				cls: 'field-container'
			})
...
function valueModal3(managedField, plugin) {
	...
	return class ValueModal extends base {
		...
		getSuggestions(query) {
			const values = getOptionsList(this.managedField).filter((o) =>
				o.toLowerCase().includes(query.toLowerCase())
			)
			if (this.addButton) {
				values.some((p) => p === query) ||
				this.managedField.options.sourceType === 'ValuesFromDVQuery' ||
				!query
	...
	if (Array.isArray(options2)) {
		...
		switch (options2.sourceType) {
			...
			case 'ValuesFromDVQuery':
				{
					const dvApi =
						(_a = managedField.plugin.app.plugins.plugins.dataview) == null ? void 0 : _a.api
					if (dvApi) {
						const dvFile = isSingleTargeted(managedField)
							? dvApi.page(managedField.target.path)
							: {}
						values = new Function(
							'dv',
							'current',
							`return ${managedField.options.valuesFromDVQuery}`
						)(dvApi, dvFile || {})
					} else {
						values = []
					}
...
function settingsModal5(Base25) {
	return class SettingModal extends Base25 {
		...
		createQueryContainer(container) {
			...
			dvQueryStringTopContainer.createEl('span', {
				text: 'Dataview Query (optional)',
				cls: 'field-option'
			})
		...
		createCustomRenderingContainer(container) {
			...
			customRenderingTopContainer.createEl('span', {
				text: "Personalise the rendering of your links' aliases with a function returning a string (<page> object is available)",
				cls: 'sub-text'
			})
			customRenderingTopContainer.createEl('code', {
				text: `function(page) { return <function using "page">; }`
			})
			const customeRenderingContainer = customRenderingTopContainer.createDiv({
				cls: 'field-container'
			})
			...
			customRendering.setPlaceholder(
				'Javascript string, the "page" (dataview page type) variable is available\nexample 1: page.file.name\nexample 2: `${page.file.name} of gender ${page.gender}`'
			)
		...
		createCustomSortingContainer(container) {
			...
			customSortingTopContainer.createEl('span', {
				text: 'Personalise the sorting order of your links with a instruction taking 2 files (a, b) and returning -1, 0 or 1',
				cls: 'sub-text'
			})
...
function getFiles(managedField) {
	...
	const getResults = (api) => {
		try {
			return new Function('dv', 'current', `return ${dvQueryString}`)(
				api,
				currentFile ? api.page(currentFile.path) : void 0
			)
		} catch (error) {
			new import_obsidian10.Notice(
				`Wrong query for field <${managedField.name}>
check your settings`,
				3e3
			)
		}
	...
	if (
		dvQueryString &&
		(dataview == null ? void 0 : dataview.settings.enableDataviewJs) &&
		(dataview == null ? void 0 : dataview.settings.enableInlineDataviewJs)
	) {
		try {
			...
			const sortingMethod = managedField.options.customSorting
				? new Function('a', 'b', `return ${managedField.options.customSorting}`)
				: function (a, b) {
						return a.basename < b.basename ? -1 : 1
					}
			return managedField.plugin.app.vault
				.getMarkdownFiles()
				.filter((f) => filesPath.includes(f.path))
				.sort(sortingMethod)
		} catch (error) {
			throw error
		}
		...
		constructor(preSelectedOptions) {
			...
			if (initialOptions && isSingleTargeted(managedField)) {
				if (Array.isArray(initialOptions)) {
					...
					initialOptions.toString().startsWith('[[')
				) {
			...
			;(_a = this.preSelectedOptions) == null
				? void 0
				: _a.forEach((item) => {
...
function settingsModal7(Base25) {
	...
	return class SettingsModal extends base {
		createSettingContainer() {
			...
			this.allowNullToggler = new import_obsidian12.ToggleComponent(allowNullValueContainer)
...
function getOptionsList2(managedField) {
	return managedField.options.allowNull
		? ['', ...getOptionsList(managedField)]
		: getOptionsList(managedField)
}
...
function settingsModal9(Base25) {
	return class SettingsModal extends Base25 {
		createSettingContainer() {
			var _a
			const container = this.optionsContainer
			if (!this.field.options.dateFormat) this.field.options.dateFormat = DefaultOptions9.dateFormat
			...
			if (this.field.type !== 'Time') {
				...
				defaultInsertAsLinkContainer.createEl('span', {
					text: 'Insert as link by default',
					cls: 'label'
				})
				defaultInsertAsLinkContainer.createDiv({ cls: 'spacer' })
				this.defaultInsertAsLink = new import_obsidian14.ToggleComponent(
					defaultInsertAsLinkContainer
				)
			...
			this.dateShiftInterval.setValue(
			...
			this.nextShiftIntervalField = new import_obsidian14.DropdownComponent(
				nextShiftIntervalFieldContainer
			)
			...
			const currentField =
				((_a = rootFields.find((_f) => _f.name === this.field.options.nextShiftIntervalField)) ==
				null
					? void 0
					: _a.id) || 'none'
			...
			this.nextShiftIntervalField.onChange((value) => {
				...
				if (value === 'none') {
					...
					const field2 =
						(_a2 = rootFields.find((_f) => _f.id === value)) == null ? void 0 : _a2.name.toString()
...
function getCycleRootFields(field2) {
	...
	if (field2.fileClassName) {
		rootFields =
			((_a = field2.plugin.fieldIndex.fileClassesFields.get(field2.fileClassName || '')) == null
				? void 0
				: _a.filter((_f) => _f.isRoot() && _f.name !== field2.name && _f.type === 'Cycle')) || []
	} else {
		rootFields = field2.plugin.presetFields.filter(
			(_f) => _f.name !== field2.name && _f.type === 'Cycle'
		)
	}
	return rootFields
}
...
function valueModal9(managedField, plugin) {
	...
	return class ValueModal extends base {
		constructor(...rest) {
			...
			this.format =
				this.managedField.options.dateFormat || this.managedField.options.defaultDateFormat
		...
		async buildInputEl(container) {
			;[this.currentShift, this.nextIntervalField, this.nextShift] = await shiftDuration(
				this.managedField
			)
			...
			this.inputEl.setPlaceholder(
				this.initialValue
					? (0, import_obsidian14.moment)(
							this.initialValue,
							this.managedField.options.dateFormat
						).format(this.managedField.options.dateFormat)
					: ''
			)
			...
			const calendarInput = wrapper.createEl('input', {
			...
			calendarInput.oninput = (e) => {
				...
				const newValue =
					this.managedField.type === 'Time'
						...
						: (0, import_obsidian14.moment)(
								(_b = e.target) == null ? void 0 : _b.value,
								'YYYY-MM-DDTHH:mm'
							).format(this.format)
		...
		buildShiftBtn(container) {
			this.shiftFromTodayBtn = new import_obsidian14.ButtonComponent(container)
		...
		async save() {
			let newValue
			if (
				this.managedField.plugin.app.plugins.enabledPlugins.has('nldates-obsidian') &&
				this.managedField.type === 'Date'
			) {
				try {
					const nldates = this.managedField.plugin.app.plugins.plugins['nldates-obsidian']
					const parsedDate = nldates.parseDate(`${this.value}`)
					newValue = parsedDate.date
						? parsedDate.moment
						: (0, import_obsidian14.moment)(`${this.value}`, this.format)
				} catch (error) {
			...
			if (newValue.isValid()) {
				...
				const sourcePath = isSingleTargeted(this.managedField)
					...
					: destPath
				const linkPath = this.managedField.plugin.app.metadataCache.getFirstLinkpathDest(
					destPath,
					sourcePath
				)
		...
		if (date.isValid()) {
			...
			if (managedField.options.defaultInsertAsLink) {
				...
				linkEl.onclick = () => {
					...
					managedField.plugin.app.workspace.openLinkText(
						linkPath,
						source ? source.path : linkPath,
						true
					)
					onClicked()
				}
...
function createDvField8(managedField, dv, p, fieldContainer, attrs = {}) {
	...
	if (file instanceof import_obsidian14.TFile && file.extension == 'md') {
		...
		shiftBtn.onclick = () => {
			if (file) shiftDate(managedField)
		}
	}
	if (!((_a = attrs == null ? void 0 : attrs.options) == null ? void 0 : _a.alwaysOn)) {
		dateBtn.hide()
	...
	if (!value) {
		...
		if (typeof value == 'string') {
			return (0, import_obsidian14.moment)(
				(_a = value.replace(/^\[\[/g, '').replace(/\]\]$/g, '').split('|').first()) == null
					? void 0
					: _a.split('/').last(),
			...
			return (0, import_obsidian14.moment)(
				(_b = value.path.replace(/^\[\[/g, '').replace(/\]\]$/g, '').split('|').first()) == null
					? void 0
					: _b.split('/').last(),
				managedField.options.dateFormat
			).isValid()
...
async function shiftDuration(managedField) {
	var _a
	if (!isSingleTargeted(managedField)) return [void 0, void 0, void 0]
	...
	const cycleField =
		(_a = managedField.plugin.fieldIndex.filesFields.get(managedField.target.path)) == null
			? void 0
			: _a.find((field2) => field2.name === cycleIntervalField)
	let nextValue
	let currentValue
	if (cycleField) {
		const eF = await Note.getExistingFieldForIndexedPath(
			managedField.plugin,
			managedField.target,
			cycleField.id
		)
		currentValue = eF == null ? void 0 : eF.value
		const cycleManager = fieldValueManager(
			managedField.plugin,
			cycleField.id,
			managedField.fileClassName,
			managedField.target,
			eF,
			cycleField.id
		)
	...
	if (
		import_obsidian14.moment.isDuration(
			import_obsidian14.moment.duration(nextShiftNumber, nextShiftPeriod)
		)
	) {
...
function getNewDateValue(managedField, currentShift) {
	...
	const [_shiftNumber, shiftPeriod] =
		(currentShift == null ? void 0 : currentShift.split(' ')) || DefaultOptions9.dateShiftInterval
	const shiftNumber = parseInt(_shiftNumber) || 1
	const _newDate = momentDate.isValid()
		? momentDate.add(shiftNumber, shiftPeriod).format(dateFormat)
		: void 0
...
function getMomentDate(managedField) {
	...
	const _dateText = _dateLink
		? (_a = _dateLink.path.split('/').last()) == null
			? void 0
			: _a.replace(/(.*).md/, '$1')
		: _date
...
async function shiftDate(managedField) {
	...
	if (!newValue) return
	const linkFile = managedField.plugin.app.metadataCache.getFirstLinkpathDest(
		linkPath || '' + newValue.format(dateFormat),
		managedField.target.path
	)
...
async function enterFieldSetting8(settingModal, field2, speed = 100) {
	...
	if (field2.options.dateFormat)
		settingModal.plugin.testRunner.insertInTextComponent(
			settingModal.dateFormatInput,
			`${field2.options.dateFormat}`
		)
	if (field2.options.dateShiftInterval)
		settingModal.plugin.testRunner.insertInTextComponent(
			settingModal.dateShiftInterval,
			`${field2.options.dateShiftInterval}`
		)
	if (field2.options.nextShiftIntervalField) {
		...
		if (field2.fileClassName) {
			cycleField =
				(_a = field2.plugin.fieldIndex.fileClassesFields.get(field2.fileClassName || '')) == null
					? void 0
					: _a.find((_f) => _f.isRoot() && _f.name !== field2.name && _f.type === 'Cycle')
		} else {
			cycleField = field2.plugin.presetFields.find(
				(_f) => _f.name !== field2.name && _f.type === 'Cycle'
			)
		}
		if (!cycleField) throw Error('Cycle field for intervals not found')
		settingModal.plugin.testRunner.selectInDropDownComponent(
			settingModal.nextShiftIntervalField,
			`${cycleField.id}`
		)
	}
}
...
async function enterFieldSetting9(settingModal, field2, speed = 100) {
	...
	if (field2.options.linkPath)
		settingModal.plugin.testRunner.insertInTextComponent(
			settingModal.dateLinkPathInput,
			`${field2.options.linkPath}`
		)
	if (field2.options.defaultInsertAsLink) settingModal.defaultInsertAsLink.toggleEl.click()
}
	...
	return class ValueModal extends base {
		constructor(...rest) {
			...
			if (isSingleTargeted(this.managedField))
				this.selectedFilePath =
					(_a = getLink(this.managedField.value, this.managedField.target)) == null
						? void 0
						: _a.path
		}
		async onChooseItem(item) {
			...
			if (dvApi && this.managedField.options.customRendering) {
				alias = new Function('page', `return ${this.managedField.options.customRendering}`)(
					dvApi.page(item.path)
				)
			}
		...
		renderSuggestion(value, el) {
			...
			if (dvApi && this.managedField.options.customRendering) {
				...
				suggestionContainer.createDiv({
					text: new Function('page', `return ${this.managedField.options.customRendering}`)(
						dvApi.page(value.item.path)
					)
				})
...
function validateValue11(managedField) {
	if (Array.isArray(managedField.value) && managedField.value.length == 1) {
		return getFiles(managedField)
			.map((f) => f.path)
			.includes(managedField.value[0].path)
	} else if (typeof managedField.value === 'string') {
		return getFiles(managedField)
			.map((f) => f.path)
			.includes(managedField.value)
	} else if (managedField.value.hasOwnProperty('path')) {
		return getFiles(managedField)
			.map((f) => f.path)
			.includes(managedField.value.path)
	} else {
		return false
	}
}
	...
	return class ValueModal extends base {
		...
		initValues() {
			...
			const pushPath = (path) => {
				...
				if (
					file instanceof import_obsidian16.TFile &&
					!this.selectedFiles.map((_f) => _f.path).includes(file.path)
				)
		...
		async save() {
			...
			const result = this.selectedFiles.map((file) => {
				...
				if (dvApi && managedField.options.customRendering) {
					alias = new Function('page', `return ${managedField.options.customRendering}`)(
						dvApi.page(file.path)
					)
				}
		...
		renderSuggestion(value, el) {
			...
			if (dvApi && managedField.options.customRendering) {
				const suggestionContainer = el.createDiv({ cls: 'item-with-add-on' })
				suggestionContainer.createDiv({
					text: new Function('page', `return ${managedField.options.customRendering}`)(
						dvApi.page(value.item.path)
					)
				})
				const filePath = suggestionContainer.createDiv({ cls: 'add-on' })
...
var FolderSuggest = class extends TextInputSuggest {
	...
	getSuggestions(inputStr) {
		...
		abstractFiles.forEach((folder) => {
			if (
				folder instanceof import_obsidian17.TFolder &&
...
// src/fields/models/abstractModels/AbstractMedia.ts
var commonMediaTypeIcon = (
	display
) => `<svg xmlns="http://www.w3.org/2000/svg" ${display === 'card' ? 'width="164" height="164"' : 'width="40" height="40"'} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-file-question">
...
function settingsModal15(Base25) {
	return class SettingModal extends Base25 {
		...
		createFolderContainer(parentNode, key) {
			...
			new FolderSuggest(this.plugin, input.inputEl)
		...
		createFoldersListContainer(parentContainer) {
			...
			;(_a = this.field.options.folders) == null
				? void 0
				: _a.forEach((folder, index) => {
						this.foldersInputComponents.push(
							this.createFolderContainer(foldersListContainer, index)
						)
					})
		...
		createEmbedTogglerContainer(container) {
			...
			new import_obsidian18.ToggleComponent(togglerContainer)
		...
		createFilesDisplaySelectorContainer(container) {
			...
			new import_obsidian18.DropdownComponent(filesDisplaySelectorContainer)
		...
		createThumbnailSizeInputContainer(container) {
			...
			thumbnailSizeInputContainer.createDiv({
				cls: 'label',
				text: 'Inline embedded thumbnail height (px): '
			})
			thumbnailSizeInputContainer.createDiv({ cls: 'spacer' })
			new import_obsidian18.TextComponent(thumbnailSizeInputContainer)
	...
	return class ValueModal extends base {
		constructor(...rest) {
			...
			this.getFiles = () => {
				...
				const files = managedField.plugin.app.vault
					.getFiles()
					.filter(
						(f) =>
							!(folders == null ? void 0 : folders.length) ||
			...
			this.managedField = managedField
			this.containerEl.addClass('metadata-menu')
			;(_a = this.containerEl.querySelector('.prompt')) == null
				? void 0
				: _a.addClass(managedField.options.display === 'card' ? 'media-as-cards' : 'media-as-list')
		}
		getItems() {
			const sortingMethod =
				new Function('a', 'b', `return ${managedField.options.customSorting}`) ||
		...
		constructor(...rest) {
			...
			if (isSingleTargeted(this.managedField)) {
				...
				if (path) {
					...
					if (
						file instanceof import_obsidian19.TFile &&
						!this.selectedFiles.map((_f) => _f.path).includes(file.path)
					)
						this.selectedFiles.push(file)
				}
	...
	return class ValueModal extends base {
		...
		initValues() {
			...
			const pushPath = (path) => {
				const file = vault.getAbstractFileByPath(path)
				if (
					file instanceof import_obsidian20.TFile &&
					!this.selectedFiles.map((_f) => _f.path).includes(file.path)
				)
					this.selectedFiles.push(file)
			}
		...
		constructor() {
			...
			this.buildColorsContainer = (container, colorList, label) => {
				...
				const toggleAltColors = () => {
					const altGroupColors =
						(colorList && colorList.filter((color) => !standardColors.includes(color))) || []
			...
			this.buildEdgeSideContainer = (container, edgeList, label) => {
				...
				sides.forEach(([side, iconName]) => {
					...
					sideTogglerContainer.onChange((value) =>
						value ? edgeList.push(side) : edgeList.remove(side)
					)
				})
			}
			this.buildLabelsContainer = (container, labels, title) => {
				...
				labels.forEach((label) => {
					...
					new import_obsidian21.ButtonComponent(labelContainer)
						.setIcon('x-circle')
			...
			this.buildNewLabelContainer = (
				currentLabelsContainer,
				currentLabelsTitle,
				newLabelContainer,
				labels,
				title
			...
			this.createDirectionContainer = (container, title) => {
				...
				Object.entries(directionOptions).forEach(([direction, label]) =>
					directionSelection.addOption(direction, label)
				)
			...
			this.createDvQueryContainer = (container, title) => {
				...
				container.createEl('span', {
					text: 'Dataview query returning a list of files (<dv> object is available)',
					cls: 'sub-text'
				})
...
function settingsModal19(Base25) {
	...
	return class SettingModal extends base {
		...
		createSettingContainer() {
			...
			container.createDiv({
				cls: 'sub-text',
				text: 'No edge selected is equivalent to all edges selected'
			})
			...
			this.createDirectionContainer(
				directionContainer,
				`Direction of the edges pointing to those nodes`
			)
			...
			this.buildEdgeSideContainer(
				edgeFromSidesContainer,
				options2.edgeFromSides,
				'Edges matchin From side:'
			)
			this.buildEdgeSideContainer(
				edgeToSidesContainer,
				options2.edgeToSides,
				'Edges matchin To side:'
			)
			this.buildLabelsContainer(edgeLabelsContainer, options2.edgeLabels, 'Edges matching labels: ')
			this.buildNewLabelContainer(
				edgeLabelsContainer,
				'Edges matching labels: ',
				newEdgeLabelContainer,
				options2.edgeLabels,
				'Add a new matching edge label'
			)
		}
	}
}
...
function settingsModal20(Base25) {
	...
	return class SettingModal extends base {
		...
		createSettingContainer() {
			...
			this.createCanvasPathContainer(canvasPathContainer)
			this.buildColorsContainer(
				groupColorsContainer,
				options2.groupColors,
				'Groups matching colors:'
			)
			this.buildLabelsContainer(
				groupLabelsContainer,
				options2.groupLabels,
...
function settingsModal21(Base25) {
	...
	return class SettingModal extends base {
		...
		createSettingContainer() {
			...
			this.createDirectionContainer(
				directionContainer,
				`Direction of the edges pointing to those groups`
			)
			this.createDvQueryContainer(
				filesFromDVQueryTopContainer,
				'Matching files connected to those groups'
			)
			this.buildColorsContainer(
				groupColorsContainer,
				options2.groupColors,
			...
			this.buildColorsContainer(
				nodeColorsContainer,
				options2.nodeColors,
				"Groups' connected nodes matching colors:"
			)
			this.buildColorsContainer(
				edgeColorsContainer,
				options2.edgeColors,
				"Groups' edges matching colors:"
			)
			...
			this.buildEdgeSideContainer(
				edgeFromSidesContainer,
				options2.edgeFromSides,
				"Groups' edges matchin From side:"
			)
			this.buildEdgeSideContainer(
				edgeToSidesContainer,
				options2.edgeToSides,
				"Groups' edges matchin To side:"
			)
			this.buildNewLabelContainer(
				groupLabelsContainer,
				groupLabelsTitle,
				newGroupLabelContainer,
				options2.groupLabels,
				'Add a new matching group name'
			)
			this.buildNewLabelContainer(
...
function arraysAsStringAreEqual(a, b) {
	const aAsArray =
		typeof a === 'string' ? a.split(',').map((v) => v.trim()) : Array.isArray(a) ? a : []
	const bAsArray =
		typeof b === 'string' ? b.split(',').map((v) => v.trim()) : Array.isArray(b) ? b : []
	return (
		aAsArray.every((item) => bAsArray.includes(item)) &&
		bAsArray.every((item) => aAsArray.includes(item))
	)
}
function renderValue(field2, pages, plugin, tFile, renderingErrors) {
	...
	switch (field2.options.outputType) {
		...
		case 'CustomBulletList' /* CustomBulletList */:
			{
				const renderingFunction = new Function(
					'page',
					`return ${field2.options.customListFunction}`
				)
				const newValuesArray =
					pages == null
						? void 0
						: pages.map((dvFile) => {
								try {
									...
									plugin.fieldIndex.fileLookupFieldsStatus.set(
										`${tFile.path}__related__${field2.fileClassName}___${field2.name}`,
										'error' /* error */
									)
									if (!renderingErrors.includes(field2.name)) renderingErrors.push(field2.name)
									return ''
								}
							})
				newValue = (newValuesArray || []).join(', ')
		...
		case 'CustomSummarizing' /* CustomSummarizing */:
			{
				...
				const summarizingFunction = new Function(
					'pages',
					customSummarizingFunction.replace(
						/\{\{summarizedFieldName\}\}/g,
						field2.options.summarizedFieldName
					)
				)
				try {
		...
		case 'BuiltinSummarizing' /* BuiltinSummarizing */:
			{
				...
				const summarizingFunction = new Function(
					'pages',
					BuiltinSummarizingFunction[builtinFunction].replace(
						/\{\{summarizedFieldName\}\}/g,
						field2.options.summarizedFieldName
					)
				)
...
function cleanRemovedLookupItemsFromIndex(plugin) {
	...
	for (let id of f.fileLookupFieldLastValue.keys()) {
		...
		const { filePath, fieldName, fileClassName } =
			((_a = id.match(matchRegex)) == null ? void 0 : _a.groups) || {}
		const file = plugin.app.vault.getAbstractFileByPath(filePath)
		if (!(file instanceof import_obsidian22.TFile)) continue
		const dvPage = f.dv.api.page(filePath)
		const currentLinks = (
			Array.isArray(dvPage[fieldName])
				? dvPage[fieldName]
				: dvPage[fieldName]
					? [dvPage[fieldName]]
					: []
		).map((p) => (p == null ? void 0 : p.path))
		...
		if (!compareArrays(currentLinks, indexedLinks)) {
			f.fileLookupFieldLastValue.set(
				id,
				currentLinks
					.filter((l) => !!l)
					.map((l) => buildMarkDownLink(plugin, file, l))
					.join(', ')
			)
		}
	}
...
async function updateLookups(plugin, forceUpdateOne, forceUpdateAll = false) {
	...
	const updatedFields = []
	await Promise.all(
		[...f.fileLookupFiles.keys()].map(async (lookupFileId) => {
			var _a, _b
			const matchRegex = /(?<filePath>.*)__related__(?<fileClassName>.*)___(?<fieldName>.*)/
			const { filePath, fieldName } =
				((_a = lookupFileId.match(matchRegex)) == null ? void 0 : _a.groups) || {}
			const file = plugin.app.vault.getAbstractFileByPath(filePath)
			...
			const field2 =
				(_b = f.filesFields.get(filePath)) == null
					? void 0
					: _b.find((field3) => field3.name == fieldName)
			if (field2) {
				...
				const shouldCheckForUpdate =
					field2.options.autoUpdate ||
					field2.options.autoUpdate === void 0 ||
					forceUpdateAll || //field is autoUpdated OR
					//field is not autoupdated and we have to check for request to update this one
					((forceUpdateOne == null ? void 0 : forceUpdateOne.file.path) === file.path &&
						(forceUpdateOne == null ? void 0 : forceUpdateOne.fieldName) === field2.name)
				const valueHasChanged =
					(!currentValue && newValue !== '') ||
					!arraysAsStringAreEqual(currentValue || '', newValue)
...
function cleanRemovedFormulasFromIndex(plugin) {
	...
	for (let id of f.fileFormulaFieldLastValue.keys()) {
		...
		const { filePath, fileClassName, fieldName } =
			((_a = id.match(matchRegex)) == null ? void 0 : _a.groups) || {}
		const existingFormulaFieldWithNameAndFileClassName =
			(_b = f.filesFields.get(filePath)) == null
				? void 0
				: _b.find(
						(field2) =>
							field2.name === fieldName &&
							((field2.fileClassName === void 0 && fileClassName === 'presetField') ||
								field2.fileClassName === fileClassName)
					)
		const dvPage = f.dv.api.page(filePath)
		if (
			dvPage === void 0 ||
			dvPage[fieldName] === void 0 ||
			!existingFormulaFieldWithNameAndFileClassName
		) {
...
async function updateFormulas(plugin, forceUpdateOne, forceUpdateAll = false) {
	...
	MDM_DEBUG &&
		console.log(
			'start update formulas',
			plugin.fieldIndex.lastRevision,
			'->',
	...
	;[...f.filesLookupAndFormulaFieldsExists].forEach(([filePath, fields]) => {
		fields
			.filter((field2) => field2.type === 'Formula')
			.forEach((field2) => {
	...
	await Promise.all(
		[...fileFormulasFields].map(async ([id, field2]) => {
			var _a2
			const matchRegex = /(?<filePath>.*)__calculated__(?<fileClassName>.*)___(?<fieldName>.*)/
			const { filePath, fileClassName, fieldName } =
				((_a2 = id.match(matchRegex)) == null ? void 0 : _a2.groups) || {}
			const shouldUpdate =
				forceUpdateAll ||
				((forceUpdateOne == null ? void 0 : forceUpdateOne.file.path) === filePath &&
					(forceUpdateOne == null ? void 0 : forceUpdateOne.fieldName) === fieldName) ||
				field2.options.autoUpdate === true
			const _file = plugin.app.vault.getAbstractFileByPath(filePath)
			if (!_file || !(_file instanceof import_obsidian23.TFile)) return
			const currentValue = `${f.fileFormulaFieldLastValue.get(id) || ''}`
			try {
				const dvFile = f.dv.api.page(filePath)
				const newValue = new Function('current, dv', `return ${field2.options.formula}`)(
					dvFile,
					f.dv.api
				).toString()
				const valueHasChanged =
					(currentValue === void 0 && newValue !== '') ||
					!arraysAsStringAreEqual(currentValue, newValue) ||
					currentValue !== newValue
				if (!valueHasChanged) {
					...
					if (!shouldUpdate) {
						...
						f.pushPayloadToUpdate(filePath, [
							{ indexedPath: field2.id, payload: { value: newValue } }
						])
	...
	return class InputSettingModal extends Base25 {
		constructor() {
			...
			this.createSettingContainer = () => {
				...
				autoUpdateTopContainer.createEl('span', {
					text: 'This could lead to latencies depending on the queries',
					cls: 'sub-text warning'
				})
				if (this.field.options.autoUpdate === void 0) this.field.options.autoUpdate = false
				...
				formulaTopContainer.createEl('span', {
					text: 'current and dv variables are available',
					cls: 'sub-text'
				})
...
function valueString22(managedField) {
	...
	const fileClassName =
		((_b =
			(_a = managedField.plugin.fieldIndex.filesFields.get(managedField.target.path)) == null
				? void 0
				: _a.find((f) => f.id === managedField.id)) == null
			? void 0
			: _b.fileClassName) || 'presetField'
	return (
		managedField.plugin.fieldIndex.fileFormulaFieldLastValue.get(
			`${managedField.target.path}__calculated__${fileClassName}___${managedField.name}`
		) || ''
	)
}
...
function actions18(plugin, field2, file, location, indexedPath) {
	...
	if (field2.options.autoUpdate === false || !plugin.settings.isAutoCalculationEnabled) {
		...
		if (
			isSuggest(location) &&
			['changed' /* changed */, 'mayHaveChanged' /* mayHaveChanged */].includes(status)
		) {
			location.options.push({
				id: `update_${name}`,
				actionLabel: `<span>Update <b>${name}</b></span>`,
			...
			location.addOption(`field_${field2.id}_udapte`, icon, () => {}, `${name} is up to date`)
		} else if (isFieldActions(location) && status === 'error' /* error */) {
			location.addOption(`field_${field2.id}_udapte`, icon, () => {}, `${name} has an error`)
		}
	} else if (isFieldActions(location)) {
...
function resolveLookups(plugin) {
	...
	;[...index.lookupQueries].forEach(([lookupName, field2]) => {
		const queryRelatedDVFiles = new Function('dv', `return ${field2.options.dvQueryString}`)(
			index.dv.api
		).values
	...
	;[...index.filesLookupAndFormulaFieldsExists].forEach(([filePath, fields]) => {
		fields
			.filter((field2) => field2.type === 'Lookup')
			.forEach((lookupField) => {
				const queryRelatedDVFiles =
					lookupQueryResults.get(
						`${lookupField.fileClassName || 'presetField'}___${lookupField.name}`
					) || []
				const fileRelatedDVFiles = queryRelatedDVFiles.filter((dvFile) => {
					...
					if (Array.isArray(targetValue)) {
						return targetValue
							.filter((v) => index.dv.api.value.isLink(v))
							.map((v) => v.path)
							.includes(filePath)
					} else {
	...
	for (let id of index.fileLookupFiles.keys()) {
		...
		const { filePath, fileClassName, fieldName } =
			((_a = id.match(matchRegex)) == null ? void 0 : _a.groups) || {}
		const existingLookFieldWithNameAndFileClassName =
			(_b = index.filesFields.get(filePath)) == null
				? void 0
				: _b.find(
						(field2) =>
							field2.name === fieldName &&
		...
		if (
			dvPage === void 0 ||
			dvPage[fieldName] === void 0 ||
			!existingLookFieldWithNameAndFileClassName
		) {
	...
	return class InputSettingModal extends Base25 {
		constructor() {
			...
			this.createSettingContainer = () => {
				...
				const dvQueryStringTopContainer = container.createDiv({ cls: 'vstacked' })
				dvQueryStringTopContainer.createEl('span', {
					text: 'Pages to look for in your vault (DataviewJS Query)',
					cls: 'label'
				})
				dvQueryStringTopContainer.createEl('span', {
					text: 'DataviewJS query of the form `dv.pages(...)`',
					cls: 'sub-text'
				})
				const dvQueryStringContainer = dvQueryStringTopContainer.createDiv({
					cls: 'field-container'
				})
				...
				targetFieldTopContainer.createEl('span', {
					text: 'field in the target pages that contains a link to the page where this lookup field is',
					cls: 'sub-text'
				})
				...
				const builtinSummarizeFunctionTopContainer = builtinOptionsContainer.createDiv({
					cls: 'vstacked'
				})
				const builtinSummarizeFunctionContainer = builtinSummarizeFunctionTopContainer.createDiv({
					cls: 'field-container'
				})
				this.field.options.builtinSummarizingFunction =
					this.field.options.builtinSummarizingFunction || Default.BuiltinSummarizing
				builtinSummarizeFunctionContainer.createEl('span', {
					text: OptionLabel.BuiltinSummarizing,
					cls: 'label'
				})
				builtinSummarizeFunctionContainer.createDiv({ cls: 'spacer' })
				const builtinSummarizeFunctionList = new import_obsidian25.DropdownComponent(
					builtinSummarizeFunctionContainer
				)
				Object.keys(BuiltinSummarizing).forEach((builtinFunction2) => {
					builtinSummarizeFunctionList.addOption(
						builtinFunction2,
						BuiltinSummarizing[builtinFunction2]
					)
				})
				const builtinOptionsDescriptionContainer = builtinSummarizeFunctionTopContainer.createDiv({
					cls: 'sub-text'
				})
				const builtinFunction = this.field.options.builtinSummarizingFunction
				builtinOptionsDescriptionContainer.setText(
					BuiltinSummarizingFunctionDescription[builtinFunction].replace(
						'{{summarizedFieldName}}',
						this.field.options.summarizedFieldName
					)
				)
				...
				builtinSummarizeFunctionList.onChange((value) => {
					...
					builtinOptionsDescriptionContainer.setText(
						BuiltinSummarizingFunctionDescription[value].replace(
							'{{summarizedFieldName}}',
							this.field.options.summarizedFieldName
						)
					)
				})
				const summarizedFieldNameTopContainer = builtinOptionsContainer.createDiv({
					cls: 'vstacked'
				})
				this.field.options.summarizedFieldName = this.field.options.summarizedFieldName
				const summarizedFieldNameContainer = summarizedFieldNameTopContainer.createDiv({
					cls: 'field-container'
				})
				summarizedFieldNameContainer.createEl('span', {
					text: 'Summarized field name',
					cls: 'label'
				})
				const summarizedFieldName = new import_obsidian25.TextComponent(
					summarizedFieldNameContainer
				)
				summarizedFieldName.inputEl.addClass('full-width')
				summarizedFieldName.inputEl.addClass('with-label')
				summarizedFieldNameTopContainer.createEl('span', {
					text: 'Name of the field containing summarized value used for the summarizing function',
					cls: 'sub-text'
				})
				...
				this.field.options.customListFunction =
					this.field.options.customListFunction || Default.CustomList
				outputRenderingFunctionTopContainer.createEl('span', {
					text: OptionLabel.CustomList,
					cls: 'label'
				})
				outputRenderingFunctionTopContainer.createEl('code', { text: OptionSubLabel.CustomList })
				const outputRenderingFunctionContainer = outputRenderingFunctionTopContainer.createDiv({
					cls: 'field-container'
				})
				const outputRenderingFunction = new import_obsidian25.TextAreaComponent(
					outputRenderingFunctionContainer
				)
				...
				this.field.options.customSummarizingFunction =
					this.field.options.customSummarizingFunction || Default.CustomSummarizing
				outputSummarizingFunctionTopContainer.createEl('span', {
					text: OptionLabel.CustomSummarizing,
					cls: 'label'
				})
				outputSummarizingFunctionTopContainer.createEl('code', {
					text: OptionSubLabel.CustomSummarizing
				})
				const outputSummarizingFunctionContainer = outputSummarizingFunctionTopContainer.createDiv({
					cls: 'field-container'
				})
				const outputSummarizingFunction = new import_obsidian25.TextAreaComponent(
					outputSummarizingFunctionContainer
				)
				...
				this.displaySelectedOutputWarningContainer(
					outputWarningContainer,
					outputTypeList.getValue()
				)
		...
		displaySelectedOutputWarningContainer(optionWarningContainer, value) {
			;[
				...
				? optionWarningContainer.show()
				: optionWarningContainer.hide()
		}
		validateOptions() {
			return true
		}
...
function actions19(plugin, field2, file, location, indexedPath) {
	...
	if (field2.options.autoUpdate === false || !plugin.settings.isAutoCalculationEnabled) {
		...
		if (
			f.fileLookupFieldLastOutputType.get(
				`${file.path}__related__${field2.fileClassName}___${field2.name}`
			) !== field2.options.outputType
		)
		...
		if (
			...
			location.addOption(`field_${field2.id}_update`, icon, () => {}, `${name} is up to date`)
		}
	} else if (isFieldActions(location)) {
		location.addOption(
			`field_${field2.id}_update`,
...
function createDvField17(managedField, dv, p, fieldContainer, attrs = {}) {
	...
	const fileClassName =
		((_b =
			(_a = managedField.plugin.fieldIndex.filesFields.get(file.path)) == null
				? void 0
				: _a.find((f) => f.name === fieldName)) == null
			? void 0
			: _b.fileClassName) || 'presetField'
	const fieldValue = dv.el(
		'span',
		managedField.plugin.fieldIndex.fileLookupFieldLastValue.get(
			`${file.path}__related__${fileClassName}___${fieldName}`
		),
		attrs
	)
	fieldContainer.appendChild(fieldValue)
	...
	return class ValueModal extends base {
		...
		buildInputEl(container) {
			...
			this.editor = new import_view2.EditorView({
				...
				extensions: [basicSetup, gutter, positionChange, this.getExtraExtensions()],
				parent: container
			})
		}
		buildFooterBtn() {
	...
	toString(doc) {
		...
		if (doc && tagEntries.length > 0 && isNode(doc.contents)) {
			...
			visit(doc.contents, (_key, node) => {
				if (isNode(node) && node.tag) tags2[node.tag] = true
			})
...
function createNode(value, tagName, ctx) {
	...
	if (isPair(value)) {
		const map2 =
			(_b = (_a = ctx.schema[MAP]).createNode) == null ? void 0 : _b.call(_a, ctx.schema, null, ctx)
	...
	const node = (tagObj == null ? void 0 : tagObj.createNode)
		...
		: typeof ((_c = tagObj == null ? void 0 : tagObj.nodeClass) == null ? void 0 : _c.from) ===
			  'function'
...
function foldFlowLines(
	...
	if (typeof indentAtStart === 'number') {
		...
		else end2 = lineWidth - indentAtStart
	}
	let split = void 0
	let prev = void 0
	let overflow = false
...
function lineLengthOverLimit(str, lineWidth, indentLength) {
	...
	for (let i = 0, start2 = 0; i < strLen; ++i) {
		if (str[i] === '\n') {
			if (i - start2 > limit) return true
			start2 = i + 1
			if (strLen - start2 <= limit) return false
		}
	}
	return true
}
...
function getTagObject(tags2, item) {
	...
	if (isScalar(item)) {
		...
		tagObj =
			(_b = match.find((t) => t.format === item.format)) != null ? _b : match.find((t) => !t.format)
	} else {
		obj = item
...
var isMergeKey = (key) =>
	key === MERGE_KEY ||
	(isScalar(key) && key.value === MERGE_KEY && (!key.type || key.type === Scalar.PLAIN))
...
var intResolve = (str, offset2, radix, { intAsBigInt }) =>
	intAsBigInt ? BigInt(str) : parseInt(str.substring(offset2), radix)
...
var binary = {
	...
	stringify({ comment, type, value }, ctx, onComment, onChompKeep) {
		...
		if (typeof Buffer === 'function') {
			str =
				buf instanceof Buffer ? buf.toString('base64') : Buffer.from(buf.buffer).toString('base64')
...
var Document = class {
	...
	setSchema(version, options2 = {}) {
		...
		else if (opt) this.schema = new Schema(Object.assign(opt, options2))
	...
	for (const token of tokens) {
		...
		switch (token.type) {
			...
			case 'anchor':
				...
				anchor = token
				if (start2 === null) start2 = token.offset
				atNewline = false
				hasSpace = false
				reqSpace = true
				break
	...
	for (const collItem of bm.items) {
		...
		if (implicitKey) {
			...
			if (keyProps.hasNewlineAfterProp || containsNewline(key)) {
				onError(
					key != null ? key : start2[start2.length - 1],
		...
		if (valueProps.found) {
			...
			const valueNode = value
				...
				: composeEmptyNode2(ctx, offset2, sep, null, valueProps, onError)
	...
	for (const { start: start2, value } of bs.items) {
		...
		if (!props.found) {
			if (props.anchor || props.tag || value) {
				...
				else onError(offset2, 'MISSING_CHAR', 'Sequence item without - indicator')
			} else {
		...
		const node = value
			...
			: composeEmptyNode2(ctx, props.end, start2, null, props, onError)
...
function resolveFlowCollection(
	...
	for (let i = 0; i < fc.items.length; ++i) {
		...
		if (!isMap2 && !sep && !props.found) {
			...
			const keyNode = key
				...
				: composeEmptyNode2(ctx, keyStart, start2, null, props, onError)
	...
	if (ee.length > 0) {
		...
		if (end2.comment) {
			if (coll.comment) coll.comment += '\n' + end2.comment
			else coll.comment = end2.comment
		}
...
function composeCollection(CN2, ctx, token, tagToken, onError) {
	...
	const res =
		(_b =
			(_a = tag.resolve) == null
				? void 0
				: _a.call(tag, coll, (msg) => onError(tagToken, 'TAG_RESOLVE_FAILED', msg), ctx.options)) !=
		null
			? _b
			: coll
...
function resolveBlockScalar(scalar, strict, onError) {
	...
	if (!header) return { value: '', type: null, comment: '', range: [start2, start2, start2] }
	...
	if (chompStart === 0) {
		...
		if (scalar.source) end3 += scalar.source.length
...
function parseBlockScalarHeader({ offset: offset2, props }, strict, onError) {
	...
	for (let i = 1; i < source.length; ++i) {
		...
		if (!chomp && (ch === '-' || ch === '+')) chomp = ch
			...
			else if (error === -1) error = offset2 + i
		}
	}
	if (error !== -1)
...
function findScalarTagByName(schema4, value, tagName, tagToken, onError) {
	...
	for (const tag of matchWithTest) if ((_a = tag.test) == null ? void 0 : _a.test(value)) return tag
...
function composeEmptyNode(
	...
	before,
	pos,
	{ spaceBefore, comment, anchor, tag, end: end2 },
	onError
) {
	const token = {
		type: 'scalar',
...
function composeAlias({ options: options2 }, { offset: offset2, source, end: end2 }, onError) {
	...
	if (alias.source === '') onError(offset2, 'BAD_ALIAS', 'Alias cannot be an empty string')
...
function composeDoc(
	options2,
	directives,
	{ offset: offset2, start: start2, value, end: end2 },
	onError
) {
	...
	doc.contents = value
		...
		: composeEmptyNode(ctx, props.end, start2, null, props, onError)
...
function _visit(path, item, visitor) {
	...
	for (const field2 of ['key', 'value']) {
		...
		if (token && 'items' in token) {
			...
			if (typeof ctrl === 'function' && field2 === 'key') ctrl = ctrl(item, path)
		}
	}
...
var Lexer = class {
	...
	*lex(source, incomplete = false) {
	...
	continueScalar(offset2) {
		...
		if (this.indentNext > 0) {
			let indent = 0
			while (ch === ' ') ch = this.buffer[++indent + offset2]
			if (ch === '\r') {
				const next = this.buffer[indent + offset2 + 1]
				if (next === '\n' || (!next && !this.atEnd)) return offset2 + indent + 1
			}
		...
		if (ch === '-' || ch === '.') {
			...
			if ((dt === '---' || dt === '...') && isEmpty(this.buffer[offset2 + 3])) return -1
		}
		return offset2
	}
	getLine() {
		...
		if (end2 === -1) return this.atEnd ? this.buffer.substring(this.pos) : null
		if (this.buffer[end2 - 1] === '\r') end2 -= 1
	...
	*parseQuotedScalar() {
		...
		if (quote === "'") {
			while (end2 !== -1 && this.buffer[end2 + 1] === "'") end2 = this.buffer.indexOf("'", end2 + 2)
		} else {
			while (end2 !== -1) {
				let n = 0
				while (this.buffer[end2 - 1 - n] === '\\') n += 1
				if (n % 2 === 0) break
	...
	constructor() {
		...
		this.linePos = (offset2) => {
			...
			while (low < high) {
				...
				if (this.lineStarts[mid] < offset2) low = mid + 1
				else high = mid
			}
			if (this.lineStarts[low] === offset2) return { line: low + 1, col: 1 }
			if (low === 0) return { line: 0, col: offset2 }
...
var Parser = class {
	...
	*step() {
		...
		if (!top2) return yield* this.stream()
		switch (top2.type) {
			case 'document':
	...
	*pop(error) {
		...
		if (!token) {
			...
			switch (top2.type) {
				...
				case 'block-seq': {
					const it = top2.items[top2.items.length - 1]
					if (it.value) top2.items.push({ start: [], value: token })
					else it.value = token
					break
				}
				case 'flow-collection': {
					const it = top2.items[top2.items.length - 1]
					if (!it || it.value) top2.items.push({ start: [], key: token, sep: [] })
			...
			if (
				(top2.type === 'document' || top2.type === 'block-map' || top2.type === 'block-seq') &&
				...
				if (
					...
					if (top2.type === 'document') top2.end = last.start
					else top2.items.push({ start: last.start })
	...
	*blockMap(map2) {
		...
		if (this.indent >= map2.indent) {
			...
			if (atNextItem && it.sep && !it.value) {
				...
				if (nl.length >= 2) start2 = it.sep.splice(nl[1])
			}
			switch (this.type) {
				case 'anchor':
				case 'tag':
...
function valueModal20(managedField, plugin) {
	...
	return class ValueModal extends base {
		...
		getExtraExtensions() {
			const yaml2 = new import_language3.LanguageSupport(
				import_language3.StreamLanguage.define(yaml)
			)
...
var FieldCommandSuggestModal = class extends import_obsidian29.FuzzySuggestModal {
	...
	renderSuggestion(item, el) {
		if (item.item.id === '__optionSeparator') {
			...
			const iconContainer = el.createDiv({ cls: 'icon-container' })
			item.item.icon
				? (0, import_obsidian29.setIcon)(iconContainer, item.item.icon)
				: (0, import_obsidian29.setIcon)(iconContainer, 'pencil')
...
var FileClassAttribute = class {
	...
	getField() {
		...
		if (Array.isArray(this.options)) {
			;(_a = this.options) == null
				? void 0
				: _a.forEach((option, index) => {
	...
	getIField() {
		...
		if (isFieldOptions([this.type, options2])) {
			...
			console.error(
				`${this.fileClassName}'s attribute [${this.name}] is not properly set. Check the fileclass settings or delete and recreate it`
			)
		}
	}
	getOptionsString(plugin) {
		const field2 = getField(this.id, this.fileClassName, this.plugin)
		if (field2) return getOptionStr(field2.type)(field2)
	}
}
...
var options = {
	...
	bookmarksGroups: {
		name: 'bookmarksGroups',
		toValue: (values) => (values.length ? values : null)
	},
	excludes: {
		name: 'excludes',
		toValue: (values) => (values.length ? values.map((attr) => attr.name) : null)
	},
...
var AddFileClassToFileModal = class extends import_obsidian30.SuggestModal {
	...
	getSuggestions(query) {
		const fileClasses = [...this.plugin.fieldIndex.fileClassesName.keys()]
			.filter((fileClassName) => {
				var _a
				return !((_a = this.plugin.fieldIndex.filesFileClasses.get(this.file.path)) == null
					? void 0
					: _a.map((fileClass) => fileClass.name).includes(fileClassName))
			})
	...
	async insertFileClassToFile(value) {
		...
		const newValue = currentFileClasses
			? [...currentFileClasses.map((fc) => fc.name), value].join(', ')
			: value
		await postValues(
			this.plugin,
			[{ indexedPath: `fileclass-field-${fileClassAlias}`, payload: { value: newValue } }],
			this.file,
			-1
		)
...
var FileClass = class {
	...
	getFileClassOptions() {
		...
		} = ((_a = this.plugin.app.metadataCache.getFileCache(this.getClassFile())) == null
			? void 0
			: _a.frontmatter) || {}
		...
		const excludes = []
		;(_b = index.fileClassesAncestors.get(this.getClassFile().basename)) == null
			? void 0
			: _b.forEach((ancestorName) => {
					var _a2
					;(_a2 = index.fileClassesName.get(ancestorName)) == null
						? void 0
						: _a2.attributes.forEach((attr) => {
								if (
									excludedNames.includes(attr.name) &&
									!excludes.map((attr2) => attr2.name).includes(attr.name)
								)
									excludes.push(attr)
							})
				})
		...
		const favoriteView =
			typeof _favoriteView === 'string' && _favoriteView !== '' ? _favoriteView : null
		const fieldsOrder = _fieldsOrder || []
		return new FileClassOptions(
			limit,
			icon,
			parent,
			excludes,
	...
	isMappedWithTag() {
		...
		try {
			...
			const mapWithTag =
				(_b =
					(_a = this.plugin.app.metadataCache.getFileCache(fileClassFile)) == null
						? void 0
						: _a.frontmatter) == null
					? void 0
					: _b.mapWithTag
	...
	getIcon() {
		const parents = [
			this.name,
			...(this.plugin.fieldIndex.fileClassesAncestors.get(this.name) || [])
		]
		let icon
		parents.some((fileClassName, i) => {
			...
			if (fileClass) {
				...
				const _icon =
					(_b =
						(_a = this.plugin.app.metadataCache.getFileCache(file)) == null
							? void 0
							: _a.frontmatter) == null
						? void 0
						: _b.icon
	...
	async missingFieldsForFileClass(file) {
		...
		const missingFields =
			this && file
				? !((_a = this.plugin.fieldIndex.fileClassesFields.get(this.name)) == null
						? void 0
						: _a.map((f) => f.id).every((id) => currentFieldsIds.includes(id)))
				: false
	...
	moveField(thisId, direction) {
		var _a
		const thisPath =
			(_a = getFileClassAttributes(this.plugin, this).find((attr) => attr.id === thisId)) == null
				? void 0
				: _a.path
		const sortedPaths = []
		...
		;[sortedPaths[thisIndex], sortedPaths[newIndex]] = [
			sortedPaths[newIndex],
			sortedPaths[thisIndex]
		]
	...
	getViewChildren(name) {
		...
		const childrenNames =
			((_b =
				(_a = this.getFileClassOptions().savedViews) == null
					? void 0
					: _a.find((_view) => _view.name === name)) == null
				? void 0
				: _b.children) || []
		...
		try {
			...
			ancestors == null
				...
				: ancestors.forEach((ancestorName) => {
						var _a2, _b2
						const ancestorFile = this.plugin.app.vault.getAbstractFileByPath(
							`${this.plugin.settings.classFilesPath}${ancestorName}.md`
						)
						const ancestor = new FileClass(this.plugin, ancestorName)
						ancestorsAttributes.set(
							ancestorName,
							getFileClassAttributes(this.plugin, ancestor, excludedFields)
						)
						if (
							ancestorFile instanceof import_obsidian30.TFile &&
							ancestorFile.extension === 'md'
						) {
							const _excludedFields2 =
								(_b2 =
									(_a2 = this.plugin.app.metadataCache.getFileCache(ancestorFile)) == null
										? void 0
										: _a2.frontmatter) == null
									? void 0
									: _b2.excludes
			...
			for (const [fileClassName, fileClassAttributes] of ancestorsAttributes) {
				this.attributes.push(
					...fileClassAttributes.filter(
						(attr) => !this.attributes.map((_attr) => _attr.name).includes(attr.name)
					)
				)
			}
		} catch (error) {
	...
	async updateAttribute(
		...
		await this.plugin.app.fileManager.processFrontMatter(file, (fm) => {
			...
			if (attr) {
				...
				field2.type = newType
				if (newOptions) field2.options = newOptions
				if (newCommand) field2.command = newCommand
				if (newDisplay) field2.display = newDisplay
				if (newStyle) field2.style = newStyle
				if (newName) field2.name = newName
				if (newPath !== void 0) field2.path = newPath
			} else {
				fm.fields.push({
					name: newName,
					type: newType,
	...
	async updateIAttribute(
		...
		const fileClass =
			attr && attr.fileClassName
				? this.plugin.fieldIndex.fileClassesName.get(attr.fileClassName)
				: this
		const file = fileClass.getClassFile()
		await this.plugin.app.fileManager.processFrontMatter(file, (fm) => {
			fm.fields = fm.fields || []
...
function buildSortedAttributes(plugin, fileClass) {
	...
	attributes.sort((a, b) => (presetOrder.indexOf(a.id) > presetOrder.indexOf(b.id) ? 1 : -1))
	...
	while (sortedAttributes.length < attributes.length) {
		...
		sortedAttributes.forEach((sAttr, parentIndex) => {
			...
			for (const attr of attributes) {
				if (
					((_a = attr.path) == null ? void 0 : _a.split('____').last()) === sAttr.id &&
					!sortedAttributes.includes(attr)
				) {
					const parentLevel = sAttr.getLevel()
					const parentSibling = sortedAttributes
						.slice(parentIndex + 1)
						.find((oAttr) => oAttr.getLevel() <= parentLevel)
					const parentSiblingIndex = parentSibling
						? sortedAttributes.indexOf(parentSibling)
		...
		if (_initial.length === sortedAttributes.length) {
			...
			new import_obsidian30.Notice(
				'Impossible to restore field hierarchy, check you fileclass configuration'
			)
			hasError = true
	...
	if (!compareArrays(presetOrder, options2.fieldsOrder)) fileClass.updateOptions(options2)
...
function getFileClassAttributes(plugin, fileClass, excludes) {
	...
	const file = fileClass.getClassFile()
	const rawAttributes =
		((_b = (_a = plugin.app.metadataCache.getFileCache(file)) == null ? void 0 : _a.frontmatter) ==
		null
			? void 0
			: _b.fields) || []
	const attributes = []
...
function getFileClassNameFromPath(settings, path) {
	...
	return (_b = (_a = path.match(fileClassNameRegex)) == null ? void 0 : _a.groups) == null
		? void 0
		: _b.fileClassName
}
...
function getSortedRootFields(plugin, fileClass) {
	var _a
	const fieldsOrder =
		fileClass.fieldsOrder || buildSortedAttributes(plugin, fileClass).map((attr) => attr.id)
	const iFinder = (f) => {
		return (id) => f.id === id
	}
	const fields =
		((_a = plugin.fieldIndex.fileClassesFields.get(fileClass.name)) == null
			? void 0
			: _a.filter((_f) => _f.isRoot())) || []
...
function sortFileFields(index, file) {
	...
	for (const fileClass of fileClasses) {
		...
		const sortedFields = order2
			? fileClassFields.sort((f1, f2) => (order2.indexOf(f1.id) < order2.indexOf(f2.id) ? -1 : 1))
			: fileClassFields
...
function indexFileClass(index, file) {
	...
	if (fileClassName) {
		try {
			...
			if (
				(fileClass.getMajorVersion() === void 0 || fileClass.getMajorVersion() < 2) &&
				index.plugin.manifest.version < '0.6.0'
			) {
			...
			if ((_b = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _b.tagNames) {
				const _tagNames =
					(_c = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _c.tagNames
				const tagNames = Array.isArray(_tagNames)
					? [..._tagNames]
					: _tagNames.split(',').map((t) => t.trim())
			...
			if ((_d = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _d.filesPaths) {
				const _filesPaths =
					(_e = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _e.filesPaths
				const filesPaths = Array.isArray(_filesPaths)
					? [..._filesPaths]
					: _filesPaths.split(',').map((f) => f.trim())
			...
			if ((_f = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _f.bookmarksGroups) {
				const _bookmarksGroups =
					(_g = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _g.bookmarksGroups
				const bookmarksGroups = Array.isArray(_bookmarksGroups)
					? [..._bookmarksGroups]
					: _bookmarksGroups.split(',').map((g) => g.trim())
				bookmarksGroups.forEach((group) =>
					index.bookmarksGroupsMatchingFileClasses.set(group, fileClass)
				)
			}
		} catch (error) {
			console.error(error)
...
var AddNewFileClassModal = class extends import_obsidian31.Modal {
	...
	buildAddFileClassForm() {
		...
		const nameErrorContainer = this.contentEl.createDiv({
			cls: 'field-error',
			text: `This ${fileClassAlias} file already exists`
		})
		cleanActions(this.contentEl, '.footer-actions')
		...
		infoContainer.setText('Alt+Enter to save')
		const saveBtn = new import_obsidian31.ButtonComponent(actionsContainer)
			.setDisabled(true)
			.setIcon('file-plus-2')
	...
	async save() {
		...
		if (classFilesPath) {
			try {
				...
				fCFile = await this.plugin.app.vault.create(
					`${classFilesPath}${fileClassName}.md`,
					'---\n---'
				)
			} catch (error) {
				openAfterCreate.remove(fileClassName)
				new import_obsidian31.Notice(
					'Something went wrong. Impossible to create this fileClass',
					3e3
				)
				return
			}
...
function genuineKeys(obj, depth = 0) {
	...
	for (const key of Object.keys(obj)) {
		...
		if (typeof obj[key] === 'object' && obj[key] !== null) {
			...
			_genuineKeys.push(
				...genuineKeys(obj[key], depth + 1).filter((k) => !_genuineKeys.includes(k))
			)
		} else if (
			!_genuineKeys
				.map((k) => k.toLowerCase().replace(/\s/g, '-'))
	...
	addDeleteButton() {
		this.addButton((b) => {
			b.setIcon('trash')
				...
				.onClick(() => {
					var _a
					const currentExistingProperty = this.plugin.presetFields.filter(
						(p) => p.id == this.field.id
					)[0]
...
var ParentSelector = class extends import_obsidian34.SuggestModal {
	...
	getSuggestions(query) {
		const parents = this.compatibleParents.filter((p) =>
	...
	renderSuggestion(parent, el) {
		...
		const display = getHierarchyDisplay(this.plugin, path, this.fieldSetting.getFileClassName())
		el.setText(display)
	}
...
function buildSettingsModal(fieldConstructor, plugin, parentSetting, parentSettingContainer) {
	return class SettingsModal extends import_obsidian34.Modal {
		...
		init() {
			...
			this.field = new (buildEmptyField(
				plugin,
				this.initialField.fileClassName,
				this.initialField.type
			))()
		...
		buildParentSelectContainer(_parent) {
			...
			const parentName = parent
				? getHierarchyDisplay(this.plugin, getParentPath(parent), this.getFileClassName())
			...
			this.parentSelectContainer
				...
				.setText(parentName)
			new import_obsidian34.ButtonComponent(this.parentSelectContainer)
		...
		createFrontmatterListDisplayContainer() {
			...
			this.frontmatterListDisplayContainer.createDiv({
				text: 'Frontmatter list display type',
				cls: 'label'
			})
			this.frontmatterListDisplayContainer.createDiv({ cls: 'spacer' })
			const frontmatterListDisplay = new import_obsidian34.DropdownComponent(
				this.frontmatterListDisplayContainer
			)
			const options2 = {}
		...
		createCommandContainer() {
			...
			;(0, import_obsidian34.setIcon)(
				iconPreview,
				((_b = this.command) == null ? void 0 : _b.icon) || 'command'
			)
		...
		createStyleSelectorContainer(parentNode) {
			...
			fieldDecorations.forEach((style) => {
				const styleBtnContainer = styleButtonsContainer.createEl(FieldHTMLTagMap[style], {
					cls: 'style-button-container'
				})
...
in the frontmatter section`)
		...
		setType(fieldType, fieldTypeLabelContainer) {
			...
			const settingsModal30 = getFieldSettingsModal(
				Field18,
				fieldType,
				plugin,
				parentSetting,
		...
		buildTypeSelectContainer() {
			...
			;(0, import_obsidian34.setIcon)(
				info.buttonEl,
				!(this.field.id && !this.isNew) ? 'shield-alert' : 'lock'
			)
...
can't be modified once saved`)
			...
			if (!this.field.id || this.isNew) {
				new import_obsidian34.ButtonComponent(this.typeSelectContainer)
			...
			this.frontmatterOnlyTypeInfoContainer = this.typeSelectContainer.createDiv({
				cls: 'tooltip-btn'
			})
			this.setTypeInfo()
			if (this.field.type) {
		...
		validateFields() {
			return (
				this.field.validateName(this.namePromptComponent, this.contentEl) && this.validateOptions()
			)
		}
			...
			if (this.fileClass) {
				...
				if (_field) {
					...
					this.plugin.presetFields = [
						...this.plugin.presetFields.filter((p) => p.id !== cEF.id),
						_field
					]
				} else {
	...
	return (
		...
		!!(
			...
			Object.keys(o1).every((k) =>
				k in o2 && typeof o1[k] === 'object' ? areOptionsEqual(o1[k], o2[k]) : o1[k] === o2[k]
			) &&
			Object.keys(o2).every((k) =>
				k in o1 && typeof o2[k] === 'object' ? areOptionsEqual(o2[k], o1[k]) : o2[k] === o1[k]
			)
		)
	)
}
	...
	addEditButton() {
		this.addButton((b) => {
			b.setIcon('pencil')
				.setTooltip('Edit')
				.onClick(() => {
					let modal = new FileClassQuerySettingsModal(
						this.plugin,
						this.containerEl,
						this,
						this.fileClassQuery
	...
	addDeleteButton() {
		this.addButton((b) => {
			b.setIcon('trash')
				...
				.onClick(() => {
					var _a
					const currentExistingFileClassQuery = this.plugin.initialFileClassQueries.find(
						(p) => p.id == this.fileClassQuery.id
					)
	...
	addMoveUpButton() {
		this.addButton((b) => {
			b.setIcon('up-chevron-glyph')
				...
				.onClick(() => {
					const currentFileClassQueryIndex = this.plugin.initialFileClassQueries
						.map((fcq) => fcq.id)
						.indexOf(this.fileClassQuery.id)
					if (currentFileClassQueryIndex > 0) {
						...
						this.plugin.initialFileClassQueries.splice(
							currentFileClassQueryIndex - 1,
							0,
							this.fileClassQuery
						)
						this.plugin.saveSettings()
...
var FileClassQuerySettingsModal = class extends import_obsidian37.Modal {
	...
	createSaveButton(container) {
		...
		b.onClick(async () => {
			...
			if (
				this.fileClassQuery.fileClassName &&
				this.fileClassQuery.name &&
				this.fileClassQuery.query
			) {
				this.saved = true
				const currentExistingFileClassQuery = this.plugin.initialFileClassQueries.filter(
					(p) => p.id == this.fileClassQuery.id
				)[0]
				if (currentExistingFileClassQuery) {
				...
				if (this.parentSetting)
					FileClassQuery_default.copyProperty(
						this.parentSetting.fileClassQuery,
						this.fileClassQuery
					)
...
var SettingGroup = class {
	...
	create() {
		...
		const settingHeaderTextContainer = settingHeaderContainer.createEl('div', {
			cls: 'text-container'
		})
		settingHeaderTextContainer.createEl('h4', { text: this.title, cls: 'section-header' })
		if (this.subTitle)
			settingHeaderTextContainer.createEl('div', {
				text: this.subTitle,
				cls: 'setting-item-description'
			})
		const settingsContainer = this.tab.containerEl.createEl('div')
		if (this.withButton) {
			const settingsContainerShowButtonContainer = settingHeaderContainer.createEl('div', {
				cls: 'setting-item-control'
			})
			this.settingsContainerShowButton = new import_obsidian38.ButtonComponent(
				settingsContainerShowButtonContainer
			)
				...
				text
					...
					.onChange(async (value) => {
						...
						values.forEach((_value) => {
							if (value.trim()) this.newValues.push(this.normalizeValue(_value.trim()))
						})
					})
				text.inputEl.rows = 2
...
var MetadataMenuSettingTab = class extends import_obsidian38.PluginSettingTab {
	...
	display() {
		...
		new import_obsidian38.Setting(globalSettings.containerEl)
			.setName('Scope')
			.setDesc(
				'Index fields in frontmatter only or in the whole note (if you use dataview inline fields). Indexing full notes could cause some latencies in vaults with large files'
			)
			.addDropdown((cb) => {
		...
		const enableAutoComplete = new import_obsidian38.Setting(globalSettings.containerEl)
		...
		const enableAutoCalculation = new import_obsidian38.Setting(globalSettings.containerEl)
		...
		const showIndexingStatus = new import_obsidian38.Setting(globalSettings.containerEl)
			...
			.addToggle((cb) => {
				...
				cb.onChange((value) => {
					...
					const indexStatusEl = document.querySelector(
						'.status-bar-item.plugin-metadata-menu .index-status'
					)
					if (!value) {
		...
		const frontmatterListDisplay = new import_obsidian38.Setting(globalSettings.containerEl)
		...
		new import_obsidian38.Setting(presetFieldsSettings.containerEl)
			...
			.addButton((button) => {
				presetFieldsSettings.addNewButton = button
					...
					.setCta()
					.onClick(async () => openSettings('', void 0, this.plugin, void 0, this.fieldsContainer))
		...
		cFS.fileClassesFolderSaveButton = new import_obsidian38.ButtonComponent(
			classFilesSettings.containerEl
		)
		...
		const path = new import_obsidian38.Setting(classFilesSettings.containerEl)
		...
		const alias = new import_obsidian38.Setting(classFilesSettings.containerEl)
			.setName('FileClass field alias')
			.setDesc(
				'Choose another name for fileClass field in frontmatter (example: Category, type, ...'
			)
			.addText((text) => {
		...
		const global = new import_obsidian38.Setting(classFilesSettings.containerEl)
			.setName('Global fileClass')
			.setDesc(
				'Choose one fileClass to be applicable to all files (even it is not present as a fileClass attribute in their frontmatter). This will override the preset Fields defined above'
			)
			.addSearch((cfs) => {
				...
				cfs
					...
					.onChange((newPath) => {
						...
						this.plugin.settings.globalFileClass = newPath
							? (_a = newPath.split('\\').pop().split('/').pop()) == null
								? void 0
								: _a.replace('.md', '')
							: ''
						this.plugin.saveSettings()
		...
		const defaultIconSetting = new import_obsidian38.Setting(classFilesSettings.containerEl)
			.setName('Default Icon')
			.setDesc('Choose a default icon for fileclasses from lucide.dev library')
			.addText((cb) => {
				cb.setValue(this.plugin.settings.fileClassIcon || DEFAULT_SETTINGS.fileClassIcon).onChange(
					(value) => {
						this.newIcon = value
		...
		;(0, import_obsidian38.setIcon)(
			iconManagerContainer,
			this.plugin.settings.fileClassIcon || DEFAULT_SETTINGS.fileClassIcon
		)
		...
		const rowPerPageSaveButton = new import_obsidian38.ButtonComponent(
			classFilesSettings.containerEl
		)
		...
		const maxRows = new import_obsidian38.Setting(classFilesSettings.containerEl)
			...
			.addText((text) => {
				text.setValue(`${this.plugin.settings.tableViewMaxRecords}`).onChange(async (value) => {
					this.newTableViewMaxRecords = parseInt(
						value || `${this.plugin.settings.tableViewMaxRecords}`
					)
		...
		const chooseFileClassAtFileCreation = new import_obsidian38.Setting(
			classFilesSettings.containerEl
		)
		...
		const autoInsertFieldsAtFileClassInsertion = new import_obsidian38.Setting(
			classFilesSettings.containerEl
		)
		...
		const showFileClassSelectInModal = new import_obsidian38.Setting(classFilesSettings.containerEl)
		...
		;[
			{
				...
				description:
					'Display an extra button to access metadata menu form after a link in reading mode',
				value: 'enableLinks'
			},
			{
				name: 'Live preview mode',
				description:
					'Display an extra button to access metadata menu form after a link in live preview',
				value: 'enableEditor'
			},
			{
				name: 'Tab header',
			...
			(s) =>
				new ButtonDisplaySetting(
					this.plugin,
					metadataMenuBtnSettings.containerEl,
					s.name,
					s.description,
					s.value,
					s.needsReload
		...
		new import_obsidian38.Setting(queryFileClassSettings.containerEl)
			...
			.addButton((button) => {
				return button
					...
					.onClick(async () => {
						let modal = new FileClassQuerySettingsModal(
							this.plugin,
							queryFileClassSettings.containerEl
						)
						modal.open()
					})
			})
...
var PathSuggestModal = class extends import_obsidian39.SuggestModal {
	...
	getSuggestions(query) {
		...
		abstractFiles.forEach((folder) => {
			if (
				folder instanceof import_obsidian39.TFolder &&
...
var BookmarksGroupSuggestModal = class extends import_obsidian39.SuggestModal {
	...
	onChooseSuggestion(item, evt) {
		var _a
		const cache =
			((_a = this.plugin.app.metadataCache.getFileCache(this.view.fileClass.getClassFile())) == null
				? void 0
				: _a.frontmatter) || {}
		const options2 = this.view.fileClass.getFileClassOptions()
...
var FileClassSetting = class {
	...
	buildSetting() {
		...
		const tooltipBtn = new import_obsidian40.ButtonComponent(toolTipBtnContainer)
			.setIcon('help-circle')
...
var FileClassSettingsView = class {
	...
	buildExcludesComponent(action) {
		...
		;(_a = this.fileClassOptions.excludes) == null
			? void 0
			: _a.forEach((field2) => {
					const fieldontainer = fieldsContainer.createDiv({ cls: 'item chip', text: field2.name })
					new import_obsidian40.ButtonComponent(fieldontainer)
						.setIcon('x-circle')
						.setClass('item-remove')
						.onClick(async () => {
							var _a2
							const excludedFields =
								(_a2 = this.fileClassOptions.excludes) == null
									? void 0
									: _a2.filter((attr) => attr.name !== field2.name)
	...
	buildExtendComponent(action) {
		...
		if (parent) {
			...
			import_obsidian40.MarkdownRenderer.render(
				this.plugin.app,
				`[[${parent.name}]]`,
				parentLinkContainer,
				path,
			...
			;(_a = parentLinkContainer.querySelector('a.internal-link')) == null
				? void 0
				: _a.addEventListener('click', (e) => {
						var _a2
						this.plugin.app.workspace.openLinkText(
							//@ts-ignore
							(_a2 = e.target.getAttr('data-href').split('/').last()) == null
								? void 0
								: _a2.replace(/(.*).md/, '$1'),
							path,
							//@ts-ignore
	...
	const selectChoices = async (collection) => {
		...
		for (const item of items) {
			...
			const choices = document.querySelectorAll(
				`#${fileClass.name}-${collection}-suggest-modal .suggestion-item`
			)
...
async function testFileClassesCreation(plugin, speed = 100) {
	...
	const fileclasses = plugin.app.vault
		.getFiles()
		.filter((f) => {
			var _a
...
async function testCreateFileClass(plugin, addBtn, fileClassDataFile, speed = 100) {
	...
	const data =
		(_a = plugin.app.metadataCache.getFileCache(fileClassDataFile)) == null
			? void 0
			: _a.frontmatter
	if (!data)
...
async function testMetadataMenuButton(plugin, speed = 100) {
	...
	if (
		!(leaf == null ? void 0 : leaf.file) ||
		!(leaf.file instanceof import_obsidian41.TFile) ||
		leaf.file.path !== testFilePath
	)
...
async function checkFileClass(plugin, file, fileClassName, speed = 100) {
	var _a
	const frontmatter =
		(_a = plugin.app.metadataCache.getFileCache(file)) == null ? void 0 : _a.frontmatter
...
async function openFieldsModal(plugin, file, speed = 100) {
	const tab = document.querySelector(
		'.workspace-tab-header[aria-label=test3_1_fileclass_frontmatter]'
	)
...
async function testNumberFieldAction(plugin, modal, speed = 100) {
	var _a
	const numberField =
		(_a = plugin.fieldIndex.filesFields.get(testFilePath)) == null
			? void 0
			: _a.find((f) => f.name === numberTest.name)
...
async function getNumberFieldResultAndTestInputField(plugin, numberField, modal, speed = 100) {
	...
	const note = await Note.buildNote(plugin, file)
	const test =
		((_a = note.existingFields.find((eF) => eF.field.id === numberField.id)) == null
			? void 0
			: _a.value) === numberField.options.step
	plugin.testRunner.log(test ? 'SUCCESS' : 'ERROR', 'Number field action from fields modal')
	const input =
		(_b = plugin.fieldIndex.filesFields.get(testFilePath)) == null
			? void 0
			: _b.find((f) => f.name === inputTest.name)
...
async function testInputUpdate(plugin, input, speed = 100) {
	...
	const note = await Note.buildNote(plugin, file)
	const test =
		((_a = note.existingFields.find((eF) => eF.field.id === input.id)) == null
			? void 0
			: _a.value) === inputTest.value
...
var TestRunner = class extends import_obsidian42.Component {
	...
	onload() {
		...
		this.registerEvent(
			this.workspace.on(
				'metadata-menu:button-built',
				async (destPath, viewTypeName, fileClassName) => {
					if (
...
function getPresetFields(plugin, speed = 100) {
	var _a, _b
	const presetFieldsFile = plugin.app.vault.getAbstractFileByPath(
		'__fixtures__/settings/presetFields.md'
	)
	...
	const fields =
		((_b =
			(_a = plugin.app.metadataCache.getFileCache(presetFieldsFile)) == null
				? void 0
				: _a.frontmatter) == null
			? void 0
			: _b['fields']) || []
...
async function enterFieldSettings(field2, container, fileClass, speed = 100) {
	...
	if (_field.fileClassName) {
		...
		savedField = (
			((_c =
				(_b = plugin.app.metadataCache.getFileCache(file)) == null ? void 0 : _b.frontmatter) ==
			null
				? void 0
				: _c.fields) || []
		).find((f) => f.name === _field.name && f.path === _field.path)
	} else {
...
var FileClassFieldSetting = class {
	...
	buildSetting() {
		...
		fieldOptionsContainer.createEl('span', {
			cls: 'description',
			text: `${fCA.getOptionsString(this.plugin)}`
		})
	}
...
var OptionsPriorityModal = class extends import_obsidian45.Modal {
	constructor(plugin, fileClassFile, field2, parentFieldSet, rowSorterComponent) {
		...
		this.containerEl.addClass('metadata-menu')
		this.initialOrder = ((_a = this.rowSorterComponent.customOrder) == null ? void 0 : _a.length)
			? this.rowSorterComponent.customOrder
			: this.getOptions()
	...
	getOptions() {
		if (this.field === 'file') {
			...
			switch (field2.type) {
				...
				case 'File': {
					const sortingMethod =
						new Function('a', 'b', `return ${field2.options.customSorting}`) ||
						function (a, b) {
							return a.basename < b.basename ? -1 : 1
						}
					try {
						return getFiles(field2)
							.sort(sortingMethod)
				...
				case 'Lookup': {
					const _values = [...this.plugin.fieldIndex.fileLookupFieldLastValue.entries()]
						.filter(([fieldId, lookupFiles]) => {
							return (
								fieldId.endsWith(
									`__related__${this.parentFieldSet.fileClass.name}___${field2.name}`
								) && lookupFiles !== ''
							)
						})
						.map(([fieldId, lookupFiles]) => lookupFiles)
						.join(',')
					return [
						...new Set(_values.split(',').map((item) => item.trim().replace(/\[\[|\]\]/g, '')))
					]
				}
				default:
					return []
	...
	renderOption(option, index) {
		const optionContainer = this.optionsContainer.createDiv({
			cls: 'suggestion-item value-container'
		})
...
var RowSorterComponent = class {
	constructor(parentFieldset, fileClass, fieldContainer, name, direction, priority, customOrder) {
		...
		this.customOrderBtn.onClick(() => {
			...
			const field2 =
				(_a = plugin.fieldIndex.fileClassesFields.get(fileClass2.name)) == null
					? void 0
					: _a.find((f) => f.isRoot() && f.name === this.name)
			new OptionsPriorityModal(
				plugin,
				fileClassFile,
				field2 || 'file',
				this.parentFieldset,
	...
	changeRowSorterPriority(priority) {
		...
		Object.keys(this.parentFieldset.rowSorters).forEach((_id) => {
			...
			if (_id == this.id) {
				...
				sorter.priority &&
				!priority &&
				sorter.priority > currentPriority
			) {
	...
	getMaxRowSorterPriority() {
		const maxPrio = Object.values(this.parentFieldset.rowSorters).reduce(
			(intermediateMax, currentSorter) => Math.max(intermediateMax, currentSorter.priority || 0),
			0
		)
		return maxPrio
	}
...
var OptionsMultiSelectModal = class extends import_obsidian47.SuggestModal {
	constructor(plugin, fileClassFile, field2, parentFieldSet) {
		...
		const id =
			this.field === 'file'
				? 'file'
				: `${this.fileClassFile.basename.replace(/.md$/, '')}____${this.field.name}`
		...
		if (this.field === 'file') {
			...
			switch (field2.type) {
				...
				case 'Select': {
					...
					const values = field3
						? getOptionsList(field3).filter((o) =>
				...
				case 'File': {
					...
					try {
						const field3 = getField(this.field.id, this.field.fileClassName, this.plugin)
						const values = (field3 ? getFiles(field3) : [])
							.sort(sortingMethod)
							.map((item) => item.basename.trim().replace(/\[\[|\]\]/g, ''))
				...
				case 'Lookup': {
					const _values = [...this.plugin.fieldIndex.fileLookupFieldLastValue.entries()]
						.filter(([fieldId, lookupFiles]) => {
							return (
								fieldId.endsWith(
									`__related__${this.fileClassFile.basename}___${this.field.name}`
								) && lookupFiles !== ''
							)
						})
						.map(([fieldId, lookupFiles]) => lookupFiles)
	...
	selectSuggestion(value, evt) {
		if (this.selectedOptions.includes(value.toString())) {
			...
			if (Object.keys(fieldStates).includes(value)) {
				...
				this.selectedOptions = [
					...this.selectedOptions.filter((o) => !Object.keys(fieldStates).includes(o))
				]
				this.selectedOptions.push(value.toString())
			}
		}
...
var CustomFilterModal = class extends import_obsidian48.Modal {
	...
	buildInput() {
		this.contentEl
			.createDiv({ cls: 'info-code' })
			.createEl('code', { text: `(value: string, current: Object): boolean => {` })
		this.contentEl.createDiv({ cls: 'info-code' }).createEl('code', { text: `/*` })
		this.contentEl.createDiv({ cls: 'info-code' }).createEl('code', {
			text: `value is the value of the file's ${this.filterComponent.name} field`
		})
		this.contentEl.createDiv({ cls: 'info-code' }).createEl('code', {
			text: `current is the current page (dv.page) if this view is embedded in a codeblock`
		})
		this.contentEl.createDiv({ cls: 'info-code' }).createEl('code', { text: `returns a boolean, ` })
		this.contentEl
			.createDiv({ cls: 'info-code' })
			.createEl('code', { text: `double quote have to be escaped like this \\", ` })
		this.contentEl.createDiv({ cls: 'info-code' }).createEl('code', { text: `example:` })
		this.contentEl
			.createDiv({ cls: 'info-code' })
			.createEl('code', { text: `return value < current.priority;` })
		this.contentEl.createDiv({ cls: 'info-code' }).createEl('code', { text: `*/` })
		const subContent = this.contentEl.createDiv({ cls: 'field-container' })
		this.filterFunctionInput = new import_obsidian48.TextAreaComponent(subContent)
...
var FilterComponent = class {
	...
	buildDropdownBtn() {
		...
		button.onclick = () => {
			...
			const field2 =
				(_a2 = plugin.fieldIndex.fileClassesFields.get(this.fileClass.name)) == null
					? void 0
					: _a2.find((f) => f.isRoot() && f.name === this.name)
			new OptionsMultiSelectModal(
				plugin,
				fileClassFile,
				field2 || 'file',
				this.parentFieldSet
	...
	buildCustomFilterBtn() {
		...
		;(_a = this.filter.inputEl.parentElement) == null
			? void 0
			: _a.appendChild(this.filterBtn.buttonEl)
...
var FieldComponent = class {
	...
	canMove(direction) {
		return !(
			(this.columnPosition === 0 && direction === 'left') ||
	...
	buildVisibilityBtn(component) {
		this.visibilityButton = new import_obsidian50.ButtonComponent(component)
	...
	buildFieldHeaderComponent() {
		...
		const priorityLabel = (
			(_a = this.parentFieldSet.rowSorters[this.id]) == null ? void 0 : _a.priority
		)
			? `(${this.parentFieldSet.rowSorters[this.id].priority})`
			: ''
		this.priorityLabelContainer = prioAndLabelContainer.createDiv({
			cls: 'priority',
			text: priorityLabel
		})
	...
	buildFieldComponent() {
		var _a
		const field2 =
			(_a = this.parentFieldSet.plugin.fieldIndex.fileClassesFields.get(
				this.parentFieldSet.fileClass.name
			)) == null
				? void 0
				: _a.find((f) => f.name === this.name)
		const debounced = (0, import_obsidian50.debounce)(
			(fieldset) => {
...
var FieldSet4 = class {
	...
	reorderFields() {
		this.fieldComponents
			...
			.forEach((field2) => this.container.appendChild(field2.container))
	}
	...
	getParams() {
		...
		const sorters = Object.entries(this.rowSorters)
			.filter(([id, s]) => {
				var _a
	...
	resetColumnManagers() {
		Object.keys(this.columnManagers).forEach((id) => {
			...
			this.fileClasses.forEach((fC) => {
				var _a, _b
				;(_b =
					(_a = this.plugin.fieldIndex.fileClassesFields.get(this.fileClass.name)) == null
						? void 0
						: _a.filter((_f) => _f.isRoot())) == null
					? void 0
					: _b.forEach((_f) => {
							fCFields.push(_f)
						})
			...
			for (const [_index, _field] of fCFields.entries()) {
				const field2 = this.fieldComponents.find(
					(f) => f.name === _field.name && f.fileClass.name === _field.fileClassName
				)
		...
		if (_name && savedViews.find((view) => view.name === _name)) {
			...
			Object.keys(this.filters).forEach((id) => {
				...
				const savedFilter =
					savedView == null
						? void 0
						: savedView.filters.find(
								(f) => f.id === id || (!f.id && f.name === filterComponent.name)
							)
				filterComponent.filter.inputEl.value =
					(savedFilter == null ? void 0 : savedFilter.query) || ''
				filterComponent.customFilter =
					(savedFilter == null ? void 0 : savedFilter.customFilter) || ''
			...
			Object.keys(this.rowSorters).forEach((id) => {
				...
				const savedSorter =
					(_a = savedView == null ? void 0 : savedView.sorters) == null
						? void 0
						: _a.find((f) => f.id === id || (!f.id && f.name === rowSorter.name))
				if (savedSorter) {
			...
			Object.keys(this.columnManagers).forEach((id) => {
				...
				const field2 = this.fieldComponents.find(
					(f) => f.id === id || (!f.id && f.name === mover.name)
				)
...
var CreateSavedViewModal = class extends import_obsidian51.Modal {
	...
	buildModal() {
		...
		const nameErrorContainer = this.contentEl.createDiv({
			cls: 'field-error',
			text: `This ${this.savedView.name} view name already exists`
		})
		cleanActions(this.contentEl, '.footer-actions')
		const actionsContainer = this.contentEl.createDiv({ cls: 'footer-actions' })
		actionsContainer.createDiv({ cls: 'spacer' })
	...
	async save() {
		...
		const options2 = this.view.fileClass.getFileClassOptions()
		options2.savedViews = [
			...(((_a = options2.savedViews) == null
				? void 0
				: _a.filter((v) => v.name !== this.savedView.name)) || []),
			this.savedView
		]
...
var FileClassDataviewTable = class {
		...
		if (dvApi) {
			try {
				...
				const fileClassFiles = [...fFC.keys()].filter((path) => {
					var _a2
					return (_a2 = fFC.get(path)) == null
						? void 0
						: _a2.some((_fileClass) => fileClassesNames.includes(_fileClass.name))
				})
				const fileFileClasses = (path) => {
					var _a2
					return (
						((_a2 = this.plugin.fieldIndex.filesFileClasses.get(path)) == null
							? void 0
							: _a2.map((_fC) => _fC.name)) || []
					)
				}
				...
				if (mutation.type == 'childList') {
					for (const node of mutation.addedNodes) {
						if (
							...
							if (fileLink) table.addClickEventToLink(fileLink)
						}
					}
				}
			}
	...
	buildBulkModifiers(observed) {
		...
		const processFilesFieldChange = (dvTable, selectedFiles, allFilesSelected, columndId) => {
			...
			const field2 =
				(_a = this.plugin.fieldIndex.fileClassesFields.get(fileClassName)) == null
					? void 0
					: _a.find((f) => f.isRoot() && f.name === fieldName)
			const files = selectedFiles
				.map((sF) => this.plugin.app.vault.getAbstractFileByPath(sF))
				.filter((f) => f instanceof import_obsidian52.TFile)
			if (field2) {
				const fieldVM = fieldValueManager(
					this.plugin,
					field2.id,
		...
		const buildCellCheckBox = (table, cell, filesCheckboxes, allFilesSelected, selectedFiles) => {
			...
			checkBox.onclick = (e) => {
				...
				if (cell.tagName === 'TH') {
					...
					const name =
						(_a = cell.querySelector('div.field-name a.internal-link')) == null
							? void 0
							: _a.getAttr('data-href')
				...
				column.onclick = (e) => {
					if (index === '0') {
						...
						for (const cell of cells) {
							...
							if (input && !input.checkVisibility()) cell.show()
							else cell.hide()
						}
					} else {
				...
				if (mutation.type == 'childList') {
					for (const node of mutation.addedNodes) {
						if (
							...
							'className' in node &&
							typeof node.className === 'string' &&
							node.className.includes('internal-embed')
						) {
							const src = node.getAttr('src')
							if (
								src &&
								node.nodeName === 'SPAN' &&
								Object.keys(extensionMediaTypes).some((extension) => src.endsWith(extension))
							) {
	...
	buildTable(tableContainer) {
		...
		tableContainer.onscroll = (e) => {
			...
			const firstFileLink =
				(_a2 = firstColl[0]) == null ? void 0 : _a2.querySelector('a.internal-link')
	...
	buidFileClassViewBtn() {
		...
		if (firstColHeader instanceof Element) {
			...
			const firstColHeaderContainer = firstColHeader.createDiv({
				cls: 'first-col-header-container'
			})
			...
			button.onclick = () => {
				const fileClassViewManager = new FileClassViewManager(
					this.plugin,
					this.fileClass,
					'tableOption',
					true,
			...
			if (checkBoxContainer)
				firstColHeaderContainer == null
					? void 0
					: firstColHeaderContainer.insertAfter(button, checkBoxContainer)
	...
	buildFilterQuery() {
		return Object.entries(this.viewConfiguration.filters)
			.map(([index, filter]) => {
				var _a
...
`
								...
								const valuesQueries = values.map(
									(val) =>
										`${valueGetter}.toString().toLowerCase().includes("${val}".toLowerCase())`
								)
...
`
	...
	buildDvJSQuery() {
		...
		const templatesFolder =
			(_a = this.plugin.app.plugins.plugins['templater-obsidian']) == null
				? void 0
				: _a.settings['templates_folder']
		dvQuery += `dv.pages()
`
...
`
	...
	buildDvJSRendering() {
		...
		const columns = this.viewConfiguration.columns
			.filter((f) => {
				var _a
				return !((_a = this.viewConfiguration.columns.find((_f) => _f.id === f.id)) == null
					? void 0
					: _a.hidden)
			})
...
var ChildrenMultiSelectModal = class extends import_obsidian53.SuggestModal {
	...
	getSuggestions(query) {
		const children = this.fileClass
			.getChildren()
			.filter((c) => !query || c.name.toLocaleLowerCase() === query.toLocaleLowerCase())
		const sortedChildren = children.sort((c1, c2) =>
			c1.path.join(' > ') < c2.path.join(' > ') ? -1 : 1
		)
...
var FileClassTableView = class {
	...
	buildLimitManager(container) {
		...
		const debounced = (0, import_obsidian54.debounce)(
			(fieldset) => fieldset.tableView.update(this.limit),
			1e3,
			true
		)
	...
	buildViewSelector() {
		...
		if (!savedViews.length) {
			...
			savedViews
				...
				.forEach((view) => this.viewSelect.addOption(view.name, view.name))
	...
	buildSavedViewRemoveButton(container) {
		...
		this.viewRemoveBtn = new import_obsidian54.ButtonComponent(btnContainer)
			.setIcon('trash')
			...
			.onClick(async () => {
				var _a
				const options2 = this.fileClass.getFileClassOptions()
				if (options2.favoriteView === this.selectedView) options2.favoriteView = null
				options2.savedViews =
					(_a = options2.savedViews) == null
						? void 0
						: _a.filter((view) => view.name !== this.selectedView)
	...
	buildSaveView(container) {
		const btnContainer = container.createDiv({ cls: 'cell' })
		this.saveViewBtn = new import_obsidian54.ButtonComponent(btnContainer)
			.setIcon('save')
...
var FileClassView = class extends import_obsidian55.ItemView {
	...
	updateDisplayView(id) {
		...
		this.menuOptions.forEach((option) =>
			option.id === id ? option.toggleActive() : option.toggleInactive()
		)
	}
	buildLayout() {
	...
	buildMenu() {
		this.menuOptions.push(
			new MenuOption(this.menu, 'tableOption', 'Table view', this.tableView.container, this)
		)
		this.menuOptions.push(
			new MenuOption(this.menu, 'fieldsOption', 'Fileclass fields', this.fieldsView.container, this)
		)
		this.menuOptions.push(
			new MenuOption(
				this.menu,
	...
	buildTableView() {
		...
		this.tableView = new FileClassTableView(
			this.component,
			this.viewContainer,
			this.tableId,
			this.fileClass,
			this.selectedView || favoriteView
		)
...
var FileClassChoiceModal = class extends import_obsidian56.SuggestModal {
	...
	getSuggestions(query) {
		...
		const values = [
			.../* @__PURE__ */ new Set([
				...index.fileClassesName.keys(),
				...index.tagsMatchingFileClasses.keys()
			])
		]
...
var FileClassViewManager = class extends import_obsidian57.Component {
	...
	async openRegisterAndIndexView(fileClass) {
		...
		this.registerEvent(
			this.plugin.app.metadataCache.on('metadata-menu:fileclass-indexed', () => {
				var _a2
				const view =
					(_a2 = this.plugin.app.workspace.getLeavesOfType(this.fileClassViewType)[0]) == null
						? void 0
						: _a2.view
				if (view) {
	...
	async build() {
		if (this.fileClass) this.openRegisterAndIndexView(this.fileClass)
			...
			if (tagsAndFileClasses.length === 1) {
				...
				const fileClass =
					index.fileClassesName.get(fileClassName) ||
					index.tagsMatchingFileClasses.get(fileClassName)
	...
	getActiveFileTagsAndFileClasses() {
		...
		const activeFilePath =
			(_a = this.plugin.app.workspace.getActiveFile()) == null ? void 0 : _a.path
		...
		if (activeFilePath && activeFilePath.endsWith('.md') && dvApi) {
			tagsAndFileClasses.push(
				...(((_c = dvApi.page(activeFilePath).etags) == null
					? void 0
					: _c.filter((tag) => [...index.tagsMatchingFileClasses.keys()].includes(tag))) || [])
			)
	...
	static async reloadViews(plugin) {
		var _a
		const registeredFileClassViews = Object.keys(plugin.app.viewRegistry.viewByType).filter((key) =>
			key.startsWith('FileClassView__')
		)
		for (const view of (await plugin.indexDB.fileClassViews.getElement('all')) || []) {
			...
			if (
				fileClassName &&
				!registeredFileClassViews.some((viewName) => viewName.includes(fileClassName))
			) {
...
var FileClassOptionsList = class {
	...
	createExtraOptionList(openAfterCreate = true) {
		...
		const missingFields =
			fileClass && this.fromFile
				? !((_b = this.plugin.fieldIndex.fileClassesFields.get(fileClass.name)) == null
						? void 0
						: _b.map((f) => f.name).every((fieldName) => currentFieldsNames.includes(fieldName)))
				: false
	...
	buildFieldOptions() {
		var _a
		;(_a = this.fileClass) == null
			? void 0
			: _a.attributes.forEach((attr) => {
					if (isMenu(this.location)) {
						this.location.addItem((item) => {
...
var OptionsList = class {
	...
	createAndOpenNodeFieldModal(node) {
		...
		const rootNode =
			(_b =
				(_a = node == null ? void 0 : node.line) == null ? void 0 : _a.getParentLineWithField()) ==
			null
				? void 0
				: _b.nodes[0]
		const indexedPath = !field2
			? (rootNode == null ? void 0 : rootNode.indexedPath) || node.indexedPath
			: node.indexedPath
	...
	createContextMenuOptionsList() {
		...
		if (isMenu2(location)) {
			...
			fileClasses.forEach((fileClass) => {
				const fileClassOptionsList = new FileClassOptionsList(
					this.plugin,
					fileClass.getClassFile(),
					location,
					this.file
				)
		...
		if (isInsertFieldCommand2(location)) {
			...
			if (this.plugin.fieldIndex.isIndexed(this.file)) {
				...
				if (dvApi) {
					...
					if (
						![...(this.plugin.fieldIndex.filesFields.get(this.file.path) || [])]
							.map((field2) => field2.name)
							.every((fieldName) => currentFieldsNames.includes(fieldName))
					) {
						this.addAllMissingFieldsAtSection()
					}
				}
			...
			const fileClasses = this.plugin.fieldIndex.filesFileClasses.get(this.file.path) || []
			if (fileClasses.length) location.options.push(separator2)
			fileClasses.forEach((fileClass) => {
				const fieldCommandSuggestModal = new FieldCommandSuggestModal(this.plugin.app)
				const fileClassOptionsList = new FileClassOptionsList(
					this.plugin,
					fileClass.getClassFile(),
					fieldCommandSuggestModal,
					this.file
				)
				fileClassOptionsList.createExtraOptionList(false)
				location.options.push({
	...
	openNoteFieldModalOption() {
		var _a
		const lastFileClassName =
			(_a = this.plugin.fieldIndex.filesFileClassesNames.get(this.file.path)) == null
				? void 0
				: _a.last()
	...
	buildFieldOptionsForMenu() {
		if (isMenu2(this.location)) {
			...
			if (view == null ? void 0 : view.editor) {
				const action = async () => {
					if (!view.file || !(view.file instanceof import_obsidian59.TFile)) return
					...
					else new import_obsidian59.Notice('No field with definition at this position', 2e3)
				}
				this.location.addItem((item) => {
			...
			: _a.existingFields
					...
					.forEach((eF) => {
						...
						if (field2) {
							...
							if (fieldVM)
								getActions(fieldVM == null ? void 0 : fieldVM.type)(
									this.plugin,
									field2,
									this.file,
									this.location,
	...
	addSectionSelectModalOption() {
		const modal = new chooseSectionModal(
			...
			this.file,
			(lineNumber, asList, asBlockquote) =>
				openFieldModal(this.plugin, this.file, void 0, lineNumber, asList, asBlockquote)
		)
		if (isMenu2(this.location)) {
			this.location.addItem((item) => {
				item.setIcon('enter')
	...
	addAllMissingFieldsAtSection() {
		const modal = new chooseSectionModal(
			...
			this.file,
			(lineNumber, asList, asBlockquote) =>
				insertMissingFields(this.plugin, this.file.path, lineNumber, asList, asBlockquote)
		)
		if (isMenu2(this.location)) {
			this.location.addItem((item) => {
				item.setIcon('battery-full')
	...
	addFieldAtTheEndOfFrontmatterOption() {
		var _a
		if (
			(_a = this.plugin.app.metadataCache.getCache(this.file.path)) == null
				? void 0
				: _a.frontmatter
		) {
			if (isMenu2(this.location)) {
				...
				this.location.options.push({
					...
					action: () => openFieldModal(this.plugin, this.file, void 0, -1, false, false),
	...
	addFieldAtCurrentPositionOption() {
		var _a
		const currentView = this.plugin.app.workspace.getActiveViewOfType(
			import_obsidian59.MarkdownView
		)
		const currentLineNumber = currentView == null ? void 0 : currentView.editor.getCursor().line
		if (
			currentLineNumber !== void 0 &&
			this.file.path == (currentView == null ? void 0 : currentView.file.path)
		) {
			const frontmatter =
				(_a = this.plugin.app.metadataCache.getFileCache(this.file)) == null
					? void 0
					: _a.frontmatter
			let lineNumber = currentLineNumber
			if (frontmatter) {
				const { start: start2, end: end2 } = getFrontmatterPosition(this.plugin, this.file)
				if (currentLineNumber >= start2.line && currentLineNumber < end2.line) lineNumber = -1
			}
			if (isMenu2(this.location)) {
				this.location.addItem((item) => {
					...
					item.onClick((evt) => {
						openFieldModal(this.plugin, this.file, void 0, lineNumber, false, false)
					})
					item.setSection('metadata-menu')
				})
				...
				this.location.options.push({
					...
					action: () => openFieldModal(this.plugin, this.file, void 0, lineNumber, false, false),
					icon: 'list-plus'
				})
			}
		}
...
function settingsModal27(Base25) {
	return class SettingsModal extends Base25 {
		createSettingContainer() {
			...
			objectDisplayTemplateTopContainer.createEl('span', {
				text: 'Object display template',
				cls: 'label'
			})
			objectDisplayTemplateTopContainer.createEl('span', {
				text: "The number of items is referenced by the keyword 'itemsCount'",
				cls: 'sub-text'
			})
			const objectDisplayTemplateContainer = objectDisplayTemplateTopContainer.createDiv({
				cls: 'field-container'
			})
...
function valueModal21(managedField, plugin) {
	...
	return class ValueModal extends base {
		...
		buildTitle(container) {
			...
			const { id, index } = getIdAndIndex(
				indexedPath == null ? void 0 : indexedPath.split('____').last()
			)
			...
			if (!this.managedField.eF) {
				...
				const field2 =
					(_a = this.managedField.plugin.fieldIndex.filesFields.get(
						this.managedField.target.path
					)) == null
						? void 0
						: _a.find((f) => f.id === upperId)
				titleContainer.setText(
					`${(field2 == null ? void 0 : field2.name) || 'unknown'}${index ? ' [' + index + ']' : ''}`
				)
			} else {
				if (index) {
		...
		buildBackButton(container) {
			new import_obsidian60.ButtonComponent(container)
				.setIcon('left-arrow')
				.onClick(async () => {
					this.onEscape()
				})
...
async function getExistingAndMissingFields(plugin, file, indexedPath) {
	var _a
	const existingFields = (await Note.getExistingFields(plugin, file)).filter(
		(eF) => eF.indexedPath && upperPath(eF.indexedPath) === indexedPath
	)
	const { id, index } = getIdAndIndex(
		indexedPath == null ? void 0 : indexedPath.split('____').last()
	)
...
function createDvField20(managedField, dv, p, fieldContainer, attrs = {}) {
	...
	editBtn.onclick = async () => {
		const file = managedField.plugin.app.vault.getAbstractFileByPath(p['file']['path'])
		const _eF =
			file instanceof import_obsidian60.TFile &&
			(await Note.getExistingFieldForIndexedPath(
				managedField.plugin,
				file,
				managedField.indexedPath
			))
		if (!_eF) return
		managedField.eF = _eF
		...
		renderSuggestion(item, el) {
			...
			if (item instanceof ExistingField2) {
				...
				const fieldVM = fieldValueManager(
					this.managedField.plugin,
					item.field.id,
					item.field.fileClassName,
					item.file,
					item,
					item.indexedPath,
					item.lineNumber
				)
				fieldVM && (fieldVM == null ? void 0 : fieldVM.value) !== ''
					? displayValue27(item.field.type)(fieldVM, valueContainer)
		...
		async onChooseSuggestion(item, evt) {
			...
			if (item instanceof ExistingField2) {
				...
				if (item.type === 'ObjectList') {
					await postValues(
						mF.plugin,
						[{ indexedPath: `${mF.indexedPath}____${item.id}`, payload: { value: '' } }],
						mF.target
					)
					this.open()
					...
					const cF = await Note.getExistingFieldForIndexedPath(
						mF.plugin,
						mF.target,
						`${mF.indexedPath}____${item.id}`
					)
					;(_a = fieldValueManager(
						mF.plugin,
						item.id,
						item.fileClassName,
						mF.target,
						cF,
						cF == null ? void 0 : cF.indexedPath,
						void 0,
						void 0,
						void 0,
						this
...
function actions23(plugin, field2, file, location, indexedPath, noteField) {
	...
	if (noteField) {
		...
		if (isFieldActions(location)) {
			location.addOption(
				`field_${field2.id}_children`,
				iconName,
				action,
				`Go to ${field2.name}'s fields`
			)
		}
	} else {
		const name = field2.name
...
function getPseudoObjectValueManagerFromObjectItem(managedField, item, previousModal) {
	...
	const itemFVM = new (FieldValueManager(
		mF.plugin,
		field2,
		mF.target,
		void 0,
	...
	return class SettingModal extends base {
		constructor() {
			...
			this.createSettingContainer = () => {
				...
				itemDisplayTemplateTopContainer.createEl('span', {
					text: 'Item display template',
					cls: 'label'
				})
				itemDisplayTemplateTopContainer.createEl('span', {
					text: "all child fields are available with their name enclosed in curly braces. Their index is referenced by the keyword 'itemIndex'",
					cls: 'sub-text'
				})
				const itemDisplayTemplateContainer = itemDisplayTemplateTopContainer.createDiv({
					cls: 'field-container'
				})
	...
	return class ValueModal extends base {
		...
		async onAdd() {
			...
			if (this.managedField.eF) {
				...
				await postValues(
					mF.plugin,
					[{ indexedPath: mF.indexedPath, payload: { value: '' } }],
					mF.target
				)
				this.close()
				this.open()
		...
		async onOpen() {
			...
			if (isSingleTargeted(mF)) {
				...
				this.objects =
					(await (_eF == null ? void 0 : _eF.getChildrenFields(mF.plugin, mF.target))) || []
			}
			super.onOpen()
		}
		getSuggestions(query = '') {
		...
		renderSuggestion(item, el) {
			...
			if (item.fields.length) {
				valueContainer.setText(
					displayItem(
						mF,
						(_a = mF.eF) == null ? void 0 : _a.value[item.indexInList],
						item.indexInList
					)
				)
			} else {
			...
			const reOpen = async () => {
				...
				if (eF) {
					;(_a = fieldValueManager(
						...
						void 0,
						void 0,
						this.managedField.previousModal
					)) == null
						? void 0
						: _a.openModal()
				}
...
function actions24(plugin, field2, file, location, indexedPath, noteField) {
	...
	if (noteField) {
		...
		if (isFieldActions(location)) {
			location.addOption(
				`field_${field2.id}_goto_${indexedPath}`,
				iconName,
				moveToObject,
				`Go to this ${name} item`
			)
			location.addOption(
				`field_${field2.id}_remove_${indexedPath}`,
				'trash',
				removeObject,
				`Remove this ${name} item`
			)
		}
	} else {
		const moveToObject = async () => {
			...
			if (_eF)
				(_a = fieldValueManager(
					plugin,
					_eF == null ? void 0 : _eF.field.id,
					_eF == null ? void 0 : _eF.field.fileClassName,
					file,
					_eF,
					_eF == null ? void 0 : _eF.indexedPath,
					void 0,
					void 0,
					void 0,
					void 0
...
async function addObjectListItem(managedField) {
	...
	if (mF.indexedPath)
		await postValues(
			mF.plugin,
			[{ indexedPath: `${mF.indexedPath}[${indexForNew}]`, payload: { value: '' } }],
			mF.target,
			-1
		)
}
...
var multiTypes = ['Multi', 'MultiFile', 'MultiMedia']
var objectTypes = ['Object', 'ObjectList']
var frontmatterOnlyTypes = ['YAML', 'Object', 'ObjectList']
var rawObjectTypes = ['YAML', 'JSON']
var rootOnlyTypes = ['Canvas', 'CanvasGroup', 'CanvasGroupLink', 'Lookup', 'Formula']
...
var InsertFieldSuggestModal = class extends import_obsidian62.FuzzySuggestModal {
	...
	getItems() {
		...
		const { start: start2, end: end2 } =
			((_a = this.plugin.app.metadataCache.getFileCache(this.file)) == null
				? void 0
				: _a.frontmatterPosition) || {}
		if (
			(start2 && end2 && start2.line <= this.lineNumber && end2.line >= this.lineNumber) ||
			this.lineNumber === -1
		) {
			return defaulOptions.concat(
				((_b = this.plugin.fieldIndex.filesFields.get(this.file.path)) == null
					? void 0
					: _b
							.filter((_f) => _f.isRoot())
							.map((field2) => {
			...
			return defaulOptions.concat(
				((_c = this.plugin.fieldIndex.filesFields.get(this.file.path)) == null
					? void 0
					: _c
							.filter((_f) => _f.isRoot() && !frontmatterOnlyTypes.includes(_f.type))
	...
	renderSuggestion(item, el) {
		...
		const iconContainer = el.createDiv({ cls: 'icon-container' })
		item.item.type
			? (0, import_obsidian62.setIcon)(iconContainer, getIcon(item.item.type))
			: (0, import_obsidian62.setIcon)(iconContainer, item.item.icon || 'plus-with-circle')
	...
	async onChooseItem(item, evt) {
		...
		if (item.actionLabel === 'Insert missing fields in frontmatter') {
			...
			new chooseSectionModal(this.plugin, this.file, (lineNumber, asList, asBlockquote) =>
				insertMissingFields(this.plugin, this.file.path, lineNumber, asList, asBlockquote)
			).open()
		} else {
			const field2 =
				(_a = this.plugin.fieldIndex.filesFields.get(this.file.path)) == null
					? void 0
					: _a.find((field3) => field3.name === item.actionLabel)
			if (field2) {
				if (objectTypes.includes(field2.type)) {
					await postValues(
						this.plugin,
						[{ indexedPath: field2.id, payload: { value: '' } }],
						this.file
					)
...
function isFieldStyle(style) {
	return (
		typeof style === 'object' &&
		Object.keys(style).every((key) => fieldDecorations.includes(FieldStyleLabel[key]))
	)
}
function field(
	plugin,
	...
	return class Field18 extends Base25 {
		...
		getChildren() {
			...
			if (this.fileClassName) {
				return (
					((_a = this.plugin.fieldIndex.fileClassesName.get(this.fileClassName)) == null
						? void 0
						: _a.attributes.map((attr) => attr.getIField()).filter((f) => !!f)
					).filter((f) => f.path.split('____').last() === this.id) || []
				)
			} else {
				return Field18.presetFields(this.plugin).filter(
					(f) => f.path.split('____').last() === this.id
				)
			}
		}
		getFirstAncestor() {
		...
		getDottedPath() {
			...
			const upperDottedPath = this.path
				.split('____')
				.map((id2) => {
					var _a
		...
		getCompatibleParents() {
			...
			if (objectTypes.includes(this.type)) {
				...
				const compatibleParents = otherObjectFields
					...
					.filter((_f) => !rootOnlyTypes.includes(this.type))
		...
		getAncestors(fieldId = this.id) {
			...
			const ancestors = []
			if (!field2 || !field2.path) return ancestors
		...
		getOtherObjectTypeFields() {
			...
			if (this.fileClassName) {
				const index = this.plugin.fieldIndex
				objectFields =
					((_a = index.fileClassesFields.get(this.fileClassName)) == null
						? void 0
						: _a.filter((field2) => objectTypes.includes(field2.type) && field2.id !== this.id)) ||
					[]
			} else {
				objectFields = this.plugin.presetFields.filter(
					(field2) => objectTypes.includes(field2.type) && field2.id !== this.id
				)
			}
		...
		validateName(textInput, contentEl) {
			...
			let error = false
			if (/^[#>-]/.test(this.name)) {
				setValidationError(textInput, 'Field name cannot start with #, >, -')
				error = true
			}
			if (this.name == '') {
				setValidationError(textInput, 'Field name can not be Empty')
				error = true
			}
			if (this.fileClassName) {
				const fields =
					(_a = this.plugin.fieldIndex.fileClassesFields.get(this.fileClassName)) == null
						? void 0
						: _a.filter(
								(_f) =>
									_f.path === this.path &&
									_f.id !== this.id &&
									_f.fileClassName === this.fileClassName
							)
				...
				const fields = this.plugin.presetFields.filter(
					(_f) => _f.path === this.path && _f.id !== this.id
				)
				if (fields == null ? void 0 : fields.map((_f) => _f.name).includes(this.name)) {
					setValidationError(
						textInput,
...
function getFieldConstructor(id, fileClassName, plugin) {
	...
	if (fileClassName) {
		const index = plugin.fieldIndex
		const fCF =
			(_a = index.fileClassesFields.get(fileClassName)) == null
				? void 0
				: _a.find((field2) => field2.id === id)
		if (!fCF) return []
		...
		return [
			buildField(
				plugin,
				plugin.settings.fileClassAlias,
				id,
				'',
				void 0,
				void 0,
		...
		if (isFieldOptions([fS.type, fS.options])) {
			...
			return [
				buildField(plugin, name, id2, path, void 0, command, display, style, ...[type, options2]),
				type
			]
		}
	}
...
function copyProperty(target, source) {
	const unbound = (value) =>
		value ? JSON.parse(JSON.stringify(value)) : value === '' ? '' : void 0
...
function getIdAndIndex(indexedId) {
	var _a
	const { id, index } = ((_a =
		indexedId == null ? void 0 : indexedId.match(/(?<id>[^\[]*)(?:\[(?<index>.*)\])?/)) == null
		? void 0
		: _a.groups) || { id: '', index: void 0 }
	return { id, index }
}
	...
	return class ManagedField extends Base25 {
		...
		async goToPreviousModal() {
			...
			if (pM && this.indexedPath && isSingleTargeted(pM.managedField)) {
				...
				const eF = await Note.getExistingFieldForIndexedPath(
					this.plugin,
					pM.managedField.target,
					pM.managedField.indexedPath
				)
				...
				if (
					upperFieldIndex !== void 0 &&
					!isNaN(parseInt(upperFieldIndex)) &&
					isSingleTargeted(this)
				) {
					const i = parseInt(upperFieldIndex)
					const upperObjectListEF = await Note.getExistingFieldForIndexedPath(
						plugin,
						this.target,
						upperPath2.replace(/\[\d+\]$/, '')
					)
					const item = ((await (upperObjectListEF == null
						? void 0
						: upperObjectListEF.getChildrenFields(this.plugin, this.target))) || [])[i]
		...
		async save(value) {
			...
			if (isSingleTargeted(this)) {
				...
				await postValues(
					plugin,
					[{ indexedPath: this.indexedPath || this.id, payload: { value: `${this.value || ''}` } }],
					this.target,
					this.lineNumber,
					this.asList,
					this.asBlockquote
...
function openFieldModal(plugin, file, fieldName, lineNumber, asList, asBlockquote) {
	...
	if (!fieldName) {
		...
		const field2 =
			(_a = plugin.fieldIndex.filesFields.get(file.path)) == null
				? void 0
				: _a.find((field3) => field3.name === fieldName)
		if (field2) {
			;(_b = fieldValueManager(
				plugin,
				field2.id,
...
function removeValidationError(textInput) {
	...
	const fieldsContainer = fieldContainer == null ? void 0 : fieldContainer.parentElement
	const fieldError =
		fieldsContainer == null ? void 0 : fieldsContainer.querySelector('.field-error')
...
var ExistingField2 = class {
	...
	async getChildrenFields(plugin, file) {
		...
		const items = []
		await Promise.all(
			this.value.map(async (value, index) => {
				const upperPath2 = `${this.indexedPath}[${index}]`
				const eFields = (await Note.getExistingFields(plugin, file)).filter(
					(eF) => eF.indexedPath && upperPath(eF.indexedPath) === upperPath2
				)
				items.push({
					fields: eFields,
					indexInList: index,
...
var LineNode = class {
	...
	defineLinePrefixAttributes(parsedContent) {
		...
		if (this.indentationLevel) {
			const parentLine = this.line.note.lines
				.filter(
					(line) =>
						line.number < this.line.number && line.indentationLevel < this.line.indentationLevel
				)
				.last()
			...
			case 'yaml':
				{
					...
					if (!yamlAttr && this.line.isNewListItem) {
						...
						const parentField = parentNode == null ? void 0 : parentNode.field
						if (
							(parentField == null ? void 0 : parentField.type) === 'ObjectList' ||
							((parentField == null ? void 0 : parentField.type) === 'Lookup' &&
								bulletListLookupTypes.includes(parentField.options.outputType))
						) {
					...
					for (const field3 of this.line.note.fields) {
						if (yamlAttr === field3.name) {
							...
							if (this.line.parentLine) {
								...
								if (parentField) {
									const parentIndexedId =
										(_a = this.line.note.existingFields.find(
											(eF) => eF.field.id === parentField.id
										)) == null
											? void 0
											: _a.indexedId
							...
							if (field3.path && indexedId !== `${field3.path}____${field3.id}`) continue
							this.indexedId = indexedId
							...
							if (field3.path) {
								...
								const parentField = parentNode == null ? void 0 : parentNode.field
								if (
									(parentField == null ? void 0 : parentField.id) ===
									field3.path.split('____').last()
								) {
									this.field = field3
									if (
										this.field &&
									...
									this.value = getValueFromIndexedPath(
										this.field,
										this.line.note.frontmatter,
										this.indexedPath
									)
			...
			case 'inline':
				{
					if (field2 && !frontmatterOnlyTypes.includes(field2.type)) {
						...
						if (
							...
							if (eF) {
								if (
									(parsedContent == null ? void 0 : parsedContent.groups) &&
									parsedContent.groups.value
								) {
	...
	createFieldNodeContent(field2, value, location, asList = false, asBlockquote = false) {
		...
		if (this.field) {
			...
			if (Array.isArray(newValue)) {
				if (
					this.field.getDisplay() === 'asList' /* asList */ ||
					(this.field.type === 'Lookup' &&
						bulletListLookupTypes.includes(this.field.options.outputType))
				) {
				...
				for (const index of nodesIndexes) {
					...
					if (parsedField2) {
						...
						new LineNode(
							this.plugin,
							this,
							this.rawContent.slice(start2, end2),
							0,
							index,
							field2,
							parsedField2.values,
						...
						const nextIndex =
							nodesIndexes[nodesIndexes.indexOf(index) + 1] || this.rawContent.length
	...
	getLastChildLine() {
		...
		for (const line of this.note.lines.filter((_l) => _l.number > this.number)) {
			...
			if (
				...
				(line.indentationLevel === this.indentationLevel &&
					((_b = (_a = line.nodes[0]) == null ? void 0 : _a.field) == null ? void 0 : _b.type) ===
						'ObjectList' &&
					!line.isNewListItem)
			) {
				lastChildLine = line
...
var Note = class {
	constructor(plugin, file) {
		...
		this.renderMultiFilesFields = (rawValue, itemRendering) => {
			const values = (rawValue.match(/\!?\[(?:\[??[^\[]*?\]\])/g) || []).map((value) =>
				itemRendering(value)
			)
		...
		this.createLine = (
			...
			if (field2) newNode.createFieldNodeContent(field2, value, position)
...
${'  '.repeat(indentationLevel + 1)}`
				...
				return [_rawValue, true, false].includes((0, import_obsidian64.parseYaml)(_rawValue)) ||
					!isNaN(parseFloat(_rawValue))
					? (0, import_obsidian64.parseYaml)(_rawValue)
					: `"${_rawValue}"`
			}
		} else {
			return ''
	...
	renderFieldValue(field2, rawValue, location) {
		...
		const indentationLevel = (field2 == null ? void 0 : field2.path)
			? field2.path.split('____').length
			: 0
		switch (location) {
			case 'yaml':
				switch (type) {
					case 'Lookup': {
						if (['BuiltinSummarizing', 'CustomSummarizing'].includes(field2.options.outputType)) {
							return this.renderValueString(rawValue, type, indentationLevel)
						} else {
							return this.renderMultiFilesFields(rawValue, (item) =>
								this.renderValueString(item, type, indentationLevel)
							)
						}
					}
					case 'Media':
					...
					case 'Multi':
						return this.renderMultiFields(rawValue, (item) =>
							this.renderValueString(item, type, indentationLevel)
						)
					case 'MultiFile':
					...
					case 'MultiMedia':
						return this.renderMultiFields(
							rawValue,
							(item) => `"${renderMediaItem('yaml', item, field2)}"`
						)
					case 'Canvas':
					...
					case void 0:
						if (
							[...ReservedMultiAttributes, this.plugin.settings.fileClassAlias].includes(
								field2.name
							)
						) {
				...
				switch (type) {
					...
					case 'MultiMedia':
						return rawValue
							...
							.filter((v) => !!v)
							.map((v) => v.trim())
							.map((v) => renderMediaItem('inline', v, field2))
							.join(', ')
					case 'Lookup': {
	...
	buildSections(content) {
			...
			if (frontmatterEnd === void 0) {
				if (!inFrontmatter) {
					if (rawLine.trim() === '') inStartingBlankLines = true
						...
						!((_a = this.frontmatterPosition) == null ? void 0 : _a.end.line) &&
						rawLine !== '---'
					)
						startsWithText = true
				}
				if (!startsWithText) {
					if (
						!inFrontmatter &&
						!((_b = this.frontmatterPosition) == null ? void 0 : _b.end.line) &&
						rawLine === '---' &&
						(inStartingBlankLines || i === 0)
					) {
						inFrontmatter = true
		...
		try {
			this.frontmatter = yamlLines.length
				? (0, import_obsidian64.parseYaml)(yamlLines.join('\n'))
				: void 0
	...
	getNodeAtPosition(position) {
		...
		const nodes =
			((_a = this.lines.find((line) => line.number === lineNumber)) == null ? void 0 : _a.nodes) ||
			[]
	...
	insertField(indexedPath, payload, lineNumber, asList = false, asBlockquote = false) {
		...
		const { id: upperFieldId, index: upperFieldIndex } = getIdAndIndex(
			upperPath2.split('____').last()
		)
		...
		if (!upperFieldIndex) {
			...
			if (!field2) return
			if (frontmatterOnlyTypes.includes(field2.type) && !this.frontmatter) this.initFrontmatter()
			const frontmatterEnd = this.frontmatterEnd()
			let insertLineNumber =
				lineNumber === 0
					? 0
					: (lineNumber !== void 0 ? Math.max(lineNumber, 0) : void 0) ||
						frontmatterEnd ||
						((_b = this.lines.last()) == null ? void 0 : _b.number) ||
						0
			if (frontmatterOnlyTypes.includes(field2.type)) insertLineNumber = frontmatterEnd
			...
			if (field2.type !== 'ObjectList') {
				...
				if ((parentField == null ? void 0 : parentField.field.type) === 'Object') {
					const parentLine =
						(_c = this.getNodeForIndexedPath(upperPath2)) == null ? void 0 : _c.line
					const lastChildLine = parentLine == null ? void 0 : parentLine.getLastChildLine()
					this.createLine(
						payload.value,
						'yaml',
						lastChildLine ? lastChildLine.number + 1 : 1,
						field2,
						asList,
						asBlockquote
					)
				...
				if (!upperNode) {
					const objectListHeaderLine = new Line(
						this.plugin,
						this,
						position,
						`${field2.name}:`,
			...
			if (lastItemLine) {
				if (
					/-(\s+)?$/.test(
						((_e = parentNode.line.objectListLines[i].last()) == null ? void 0 : _e.rawContent) ||
							''
					) &&
					field2
				) {
	...
	async createOrUpdateFields(fields, lineNumber, asList = false, asBlockquote = false) {
		fields.forEach((field2) => {
			...
			if (node && node.field) {
				node.createFieldNodeContent(
					node.field,
					field2.payload.value,
					node.line.position,
					asList,
					asBlockquote
				)
...
async function insertMissingFields(
	...
	const filteredClassFields = fileClassName
		? ((_a = plugin.fieldIndex.fileClassesFields.get(fileClassName)) == null
				? void 0
				: _a.filter((field2) => field2.fileClassName === fileClassName)) || void 0
		: void 0
	...
	if (!indexedPath) {
		fields == null
			...
			: fields
					.filter(
						(field2) =>
							field2.isRoot() && !note.existingFields.map((_f) => _f.field.id).includes(field2.id)
					)
					.filter((field2) =>
						filteredClassFields ? filteredClassFields.map((f2) => f2.id).includes(field2.id) : true
					)
					.reverse()
					.forEach((field2) => {
		...
		const existingFields = note.existingFields.filter((_f) => {
			...
			return (
				(upperIndexedIdsInPath == null ? void 0 : upperIndexedIdsInPath.join('____')) ===
				indexedPath
			)
		})
		const missingFields =
...
var FieldsModal = class extends import_obsidian65.Modal {
	...
	buildFieldContainer(container, field2, value, indexedPath) {
		...
		const fieldNameContainer = fieldNameWrapper.createDiv({
			text: `${field2.name}`,
			cls: 'field-item field-name'
		})
	...
	buildFieldSetting(container, field2, fieldNameContainer, isObjectListItem = false) {
		...
		const fileClass = field2.fileClassName
			? this.plugin.fieldIndex.fileClassesName.get(field2.fileClassName)
			: void 0
		if (fileClass) {
			fieldNameContainer.addClass(
				`fileClassField__${fileClass.name.replace('/', '___').replaceAll(' ', '_')}`
			)
		}
		const fieldSettingContainer = fieldSettingsWrapper.createDiv({
			cls: 'field-item field-setting'
		})
		const fieldSettingBtn = new import_obsidian65.ButtonComponent(fieldSettingContainer)
		fieldSettingBtn.setIcon('gear')
		fieldSettingBtn.setTooltip(
			`${field2.fileClassName ? field2.fileClassName + ' > ' : 'Preset Field > '} ${field2.name} settings`
		)
		fieldSettingBtn.onClick(() => {
			const _fileClass = field2.fileClassName
				? this.plugin.fieldIndex.fileClassesName.get(field2.fileClassName)
				: void 0
			const fileClassAttribute =
				_fileClass == null ? void 0 : _fileClass.attributes.find((attr) => attr.id === field2.id)
		...
		fieldTypeContainer.createDiv({
			text: `${field2.type}${isObjectListItem ? ' item' : ''}`,
			cls: `chip field-type ${field2.colorClass}`
		})
	}
	buildFieldValue(container, field2, fieldVM, value) {
		...
		const fieldValueContainer = fieldValueWrapper.createDiv({
			cls: ![void 0, null, ''].includes(value)
				? 'field-item field-value'
				: 'field-item field-value emptyfield'
		})
		if (value === null || value === '') {
		...
		if (this.existingFields.map((_f) => _f.field.id).includes(field2.id) && fieldVM) {
			...
			fieldBtn.onClick(async () => {
				...
				if (objectTypes.includes(field2.type) && this.note) {
					await postValues(
						this.plugin,
						[{ indexedPath: `${newIndexedPath}`, payload: { value: '' } }],
						this.file
					)
	...
	buildFileClassManager(container) {
		const fileClasses = this.plugin.fieldIndex.filesFileClasses.get(this.file.path) || []
		fileClasses.forEach((fileClass) => {
			const fileClassManagerContainer = container.createDiv({
				cls: 'fields-inheritance-manager-container'
			})
			...
			ancestors.forEach(async (fileClassName, i) => {
				...
				if (_fileClass) {
					const fileClassOptionsContainer = fileClassManagerContainer.createDiv({
						cls: 'fileclass-manager-container'
					})
					const fileClassNameContainer = fileClassOptionsContainer.createDiv({
						cls: 'name',
						text: _fileClass.name
					})
					fileClassNameContainer.setAttr(
						'id',
						`fileClass__${_fileClass.name.replace('/', '___').replace(' ', '_')}`
					)
					if (await _fileClass.missingFieldsForFileClass(this.file)) {
						const fileClassInsertMissingFieldsInFrontmatterBtn =
							new import_obsidian65.ButtonComponent(fileClassOptionsContainer)
						fileClassInsertMissingFieldsInFrontmatterBtn.setIcon('align-vertical-space-around')
						fileClassInsertMissingFieldsInFrontmatterBtn.setTooltip(
							`Insert missing fields for ${_fileClass.name}`
						)
						...
						const fileClassInsertMissingFieldsBtn = new import_obsidian65.ButtonComponent(
							fileClassOptionsContainer
						)
						fileClassInsertMissingFieldsBtn.setIcon('log-in')
						fileClassInsertMissingFieldsBtn.setTooltip(
							`Insert missing fields for ${_fileClass.name}`
						)
					...
					const fileClassAddAttributeBtn = new import_obsidian65.ButtonComponent(
						fileClassOptionsContainer
					)
					...
					const fileClassFieldsContainers = this.containerEl.querySelectorAll(
						`[class*="fileClassField__${fileClassName.replace('/', '___').replace(' ', '_')}"]`
					)
					...
					fileClassNameContainer.onmouseover = () => {
						this.containerEl
							.querySelectorAll(
								`.field-item.field-name.fileClassField__${fileClassName.replace('/', '___').replace(' ', '_')}`
							)
							.forEach((cont) => {
	...
	buildInsertMissingFieldsBtn() {
		...
		const insertMissingFieldsInFrontmatterBtn = new import_obsidian65.ButtonComponent(
			insertMissingFieldsContainer
		)
		...
		insertMissingFieldsInFrontmatterBtn.onClick(() => {
			...
			if (!this.indexedPath) {
				...
				const field2 =
					(_a = this.note.getExistingFieldForIndexedPath(this.indexedPath)) == null
						? void 0
						: _a.field
				const fileClass = (field2 == null ? void 0 : field2.fileClassName)
					? this.plugin.fieldIndex.fileClassesName.get(field2.fileClassName)
					: void 0
				insertMissingFields(
					this.plugin,
		...
		const insertMissingFieldsBtn = new import_obsidian65.ButtonComponent(
			insertMissingFieldsContainer
		)
	...
	buildFieldsContainer() {
		...
		const { id, index } = getIdAndIndex(
			(_a = this.indexedPath) == null ? void 0 : _a.split('____').last()
		)
		if (
			this.indexedPath &&
			((_b = this.note.fields.find((_f) => _f.id === id)) == null ? void 0 : _b.type) ===
				'ObjectList' &&
			index === void 0
		) {
			const field2 = this.note.fields.find((_f) => _f.id === id)
			const items =
				((_c = this.note.existingFields.find((eF) => eF.indexedPath === this.indexedPath)) == null
					? void 0
					: _c.value) || []
			items.forEach((item, index2) =>
				this.buildObjectListItemContainer(fieldsContainer, field2, `${this.indexedPath}[${index2}]`)
			)
			...
			const fields = [
				...
				...this.missingFields.sort((f1, f2) =>
					sortedIds.indexOf(f1.id) < sortedIds.indexOf(f2.id) ? -1 : 1
				)
			]
	...
	plugin.addCommand({
		...
		checkCallback: (checking) => {
			...
			const inFileClass = !!(
				...
				view.file.path.startsWith(classFilesPath)
			)
			if (checking) {
				return inFileClass
			}
...
function insertFieldAtPositionCommand(plugin) {
	...
	plugin.addCommand({
		...
		checkCallback: (checking) => {
			...
			const inFile = !!(
				(view == null ? void 0 : view.file) &&
				(!classFilesPath || !view.file.path.startsWith(classFilesPath))
			)
			if (checking) {
				return inFile
			}
						...
						case 'source':
							{
								...
								if (node) optionsList.createAndOpenNodeFieldModal(node)
								else new import_obsidian66.Notice('No field with definition at this position', 2e3)
							}
							break
						case 'preview': {
							...
							if (focusedElement instanceof HTMLElement) {
								...
								const field2 =
									key &&
									((_a = plugin.fieldIndex.filesFields.get(view.file.path)) == null
										? void 0
										: _a.find((_f) => _f.isRoot() && _f.name === key))
								if (field2) {
									...
									const node = note.getNodeForIndexedPath(
		...
		checkCallback: (checking) => {
			...
			if (inFile) {
				;(async function () {
					...
					if (
						![...(plugin.fieldIndex.filesFields.get(file.path) || [])]
							.map((field2) => field2.name)
							.every((fieldName) => existingFieldsNames.includes(fieldName))
					) {
						new chooseSectionModal(plugin, file, (lineNumber, asList, asBlockquote) =>
							insertMissingFields(plugin, file.path, lineNumber, asList, asBlockquote)
						).open()
					}
				})()
			}
...
function insertFieldCommand(plugin, command, field2, fileClassName) {
	plugin.addCommand({
		...
		checkCallback: (checking) => {
			...
			const fileClasses = (view == null ? void 0 : view.file)
				? plugin.fieldIndex.filesFileClasses.get(view == null ? void 0 : view.file.path)
				: void 0
			const belongsToView =
				field2 !== void 0 &&
				!!(view == null ? void 0 : view.file) &&
			...
			if ((view == null ? void 0 : view.file) && field2) {
				new chooseSectionModal(plugin, view.file, (lineNumber, asList, asBlockquote) => {
					var _a
					;(_a = fieldValueManager(
						plugin,
						field2.id,
...
function insertFieldsCommand(plugin) {
	...
	plugin.presetFields.forEach((f) => {
		if (f.command && f.isRoot()) fields.push({ field: f, fileClassName: void 0 })
	})
...
function openFileclassViewCommand(plugin) {
	plugin.addCommand({
		...
		checkCallback: (checking) => {
			...
			const fileClass = activeFilePath
				? plugin.fieldIndex.fileClassesPath.get(activeFilePath)
				: void 0
			...
			if (inFile) {
				...
				if (inFile && file instanceof import_obsidian66.TFile && file.extension === 'md') {
					const lookupFields =
						(_a = plugin.fieldIndex.filesFields.get(file.path)) == null
							? void 0
							: _a.filter((field2) => field2.type === 'Lookup')
					lookupFields == null
						? void 0
						: lookupFields.forEach(async (field2) => {
								await updateLookups(plugin, { file, fieldName: field2.name })
			...
			if (inFile) {
				...
				if (inFile && file instanceof import_obsidian66.TFile && file.extension === 'md') {
					const formulaFields =
						(_a = plugin.fieldIndex.filesFields.get(file.path)) == null
							? void 0
							: _a.filter((field2) => field2.type === 'Formula')
					formulaFields == null
						? void 0
						: formulaFields.forEach(async (field2) => {
...
var ContextMenu = class extends import_obsidian67.Component {
	...
	buildOptions(file, menu) {
		...
		if (file instanceof import_obsidian67.TFile && file.extension === 'md') {
			if (
				!import_obsidian67.Platform.isMobile &&
				(0, import_obsidian67.requireApiVersion)('0.16.0')
			) {
				if (classFilesPath && file.path.startsWith(classFilesPath)) {
					...
					menu.setSectionSubmenu(`metadata-menu-fileclass.${fileClassName}.fileclass-fields`, {
						title: 'Manage fields',
						icon: 'wrench'
					})
				} else {
					...
					fileClasses.forEach((fileClass) => {
						menu.setSectionSubmenu(`metadata-menu-fileclass.${fileClass.name}.fileclass-fields`, {
							title: `Manage ${fileClass.name} fields`,
							icon: 'wrench'
						})
					})
				}
...
function setLinkMetadataFormButton(plugin, link, destPath, viewTypeName, fileClassName) {
	...
	if (link.classList.contains('metadata-menu-button-hidden')) return
	switch (viewTypeName) {
	...
	if (classFilessPath && fileClass) {
		...
		const el = link.nextElementSibling
		if (!(el == null ? void 0 : el.hasClass('fileclass-icon'))) {
			const metadataMenuBtn = plugin.app.workspace.containerEl.createEl('a', {
				cls: 'metadata-menu fileclass-icon'
			})
			setStatus(metadataMenuBtn)
			if (metadataMenuBtn) {
				;(0, import_obsidian68.setIcon)(metadataMenuBtn, icon)
				;(_a = link.parentElement) == null
					? void 0
					: _a.insertBefore(metadataMenuBtn, link.nextSibling)
		...
		if (fileClass2) {
			...
			if (!(el == null ? void 0 : el.hasClass('fileclass-icon'))) {
				...
				if (metadataMenuBtn) {
					...
					;(_b = link.parentElement) == null
						? void 0
						: _b.insertBefore(metadataMenuBtn, link.nextSibling)
...
function updateLinkMetadataMenuFormButton(app2, plugin, link, viewTypeName, source) {
	...
	if (dest) {
		const fileClassName =
			(_b = plugin.fieldIndex.filesFileClassesNames.get(dest.path)) == null ? void 0 : _b.last()
		setLinkMetadataFormButton(
			plugin,
			link,
			dest.path.replace(/(.*).md/, '$1'),
			viewTypeName,
			fileClassName
		)
	}
...
function updateDivExtraAttributes(app2, plugin, link, viewTypeName, sourceName, _linkName) {
	...
	switch (viewTypeName) {
		case 'file-explorer':
			{
				const dataPath =
					(_a = link == null ? void 0 : link.parentElement) == null ? void 0 : _a.dataset.path
				if (dataPath) {
					const fileClassName =
						(_b = plugin.fieldIndex.filesFileClassesNames.get(dataPath)) == null
							? void 0
							: _b.last()
					setLinkMetadataFormButton(
						plugin,
						link,
						dataPath.replace(/(.*).md/, '$1'),
						viewTypeName,
						fileClassName
					)
				}
		...
		case 'tabHeader':
			{
				if (sourceName) {
					const fileClassName =
						(_c = plugin.fieldIndex.filesFileClassesNames.get(sourceName)) == null
							? void 0
							: _c.last()
					setLinkMetadataFormButton(
						plugin,
						link,
						sourceName.replace(/(.*).md/, '$1'),
						viewTypeName,
						fileClassName
					)
				}
		...
		case 'outgoing-link':
			{
				...
				if (dest) {
					const fileClassName =
						(_d = plugin.fieldIndex.filesFileClassesNames.get(`${dest}.md`)) == null
							? void 0
							: _d.last()
		...
		default:
			{
				...
				const dest =
					...
					app2.metadataCache.getFirstLinkpathDest(
						(0, import_obsidian68.getLinkpath)(linkName),
						sourceName
					)
				if (dest) {
					const fileClassName =
						(_e = plugin.fieldIndex.filesFileClassesNames.get(dest.path)) == null
							? void 0
							: _e.last()
					setLinkMetadataFormButton(
						plugin,
...
function updatePropertiesPane(propertiesEl, file, app2, plugin) {
	...
	const frontmatter =
		(_a = app2.metadataCache.getCache(file.path)) == null ? void 0 : _a.frontmatter
	if (!!frontmatter) {
		...
		for (let i = 0; i < nodes.length; ++i) {
			...
			const keyEl =
				(_e =
					(_d =
						(_c =
							(_b = el == null ? void 0 : el.parentElement) == null ? void 0 : _b.parentElement) ==
						null
							? void 0
		...
		for (let i = 0; i < singleNodes.length; ++i) {
			...
			const keyEl =
				(_h =
					(_g =
						(_f = el == null ? void 0 : el.parentElement) == null ? void 0 : _f.parentElement) ==
					null
						? void 0
						: _g.parentElement) == null
					? void 0
					: _h.children[0].children[1]
			const key = keyEl.value
		...
		if (leaf.view instanceof import_obsidian68.MarkdownView && leaf.view.file) {
			...
			if ((cachedFile == null ? void 0 : cachedFile.links) && settings.enableLinks) {
				cachedFile.links.forEach((link) => {
					...
					if (dest) {
						const fileClassName =
							(_a2 = plugin.fieldIndex.filesFileClassesNames.get(dest.path)) == null
								? void 0
								: _a2.last()
								...
								if ((mdView.file && isLink && !isAlias2 && !isPipe) || isMDUrl) {
									...
									if (file) {
										...
										if (
											...
											file.path.startsWith(plugin.settings.classFilesPath)
										)
											fileClassName = file.basename
										else
											fileClassName =
												(_a = plugin.fieldIndex.filesFileClassesNames.get(file.path)) == null
													? void 0
													: _a.last()
										if (fileClassName) {
...
var ExtraButton = class extends import_obsidian70.Component {
	...
	onload() {
		...
		this.registerEvent(
			this.plugin.app.metadataCache.on(
				'changed',
				(0, import_obsidian70.debounce)(this.updateLinks, 100, true)
			)
		)
		this.registerEvent(
			this.plugin.app.metadataCache.on(
				'metadata-menu:indexed',
				(0, import_obsidian70.debounce)(this.updateLinks, 100, true)
			)
		)
		this.registerEvent(
			this.plugin.app.workspace.on(
				'layout-change',
				(0, import_obsidian70.debounce)(this.updateLinks, 10, true)
			)
		)
		this.registerEvent(
			this.plugin.app.workspace.on('window-open', (window2, win) =>
				this.initModalObservers(window2.getContainer().doc)
			)
		)
		this.registerEvent(
			this.plugin.app.workspace.on('layout-change', () => this.initViewObservers())
		)
		this.registerEvent(
			this.plugin.app.internalPlugins
				.getPluginById('bookmarks')
				.instance.on('changed', (0, import_obsidian70.debounce)(this.updateLinks, 100, true))
		)
	}
	initViewObservers() {
		...
		if (
			(_f =
				(_e =
					(_d =
						(_c =
							(_b = (_a = this.plugin.app) == null ? void 0 : _a.internalPlugins) == null
								? void 0
								: _b.plugins) == null
							? void 0
							: _c.backlink) == null
				...
				records.forEach((mutation) => {
					if (mutation.type === 'childList') {
						mutation.addedNodes.forEach((n) => {
							if (
								'className' in n && // @ts-ignore
								((n.className.includes('modal-container') &&
									this.plugin.settings.enableQuickSwitcher) ||
									(n.className.includes('suggestion-container') &&
										this.plugin.settings.enableSuggestor))
							) {
								let selector =
	...
	updateContainer(container, selector, viewTypeName) {
		...
		for (let i = 0; i < nodes.length; ++i) {
			const el = nodes[i]
			const isCanvasFileLink =
				(_b = (_a = el.parentElement) == null ? void 0 : _a.getAttr('data-path')) == null
					? void 0
					: _b.includes('.canvas')
			if (!isCanvasFileLink) {
					...
					mutation.addedNodes.forEach((n) => {
						if ('className' in n) {
							if (
								...
								typeof n.className.includes === 'function' &&
								n.className.includes(parent_class)
							) {
...
async function updateCanvas(plugin, forceUpdateOne) {
	...
	const f = plugin.fieldIndex
	const dvApi = (_a = plugin.app.plugins.plugins.dataview) == null ? void 0 : _a.api
	const canvases = forceUpdateOne
		? [forceUpdateOne.canvas]
		: plugin.app.vault.getFiles().filter((t) => t.extension === 'canvas')
	...
	const targetNode = (direction, edge, nodes, currentNode) => {
		switch (direction) {
			...
			case 'bothsides':
				return nodes.find(
					(node) =>
						node.id !== currentNode.id && (node.id === edge.toNode || node.id === edge.fromNode)
				)
			default:
				return void 0
		}
	...
	const resolveFieldLinksForNode = (field2, targetFilePath, edges, nodes, node, cumulativeSet) => {
		...
		const matchingFiles =
			filesFromDVQuery && dvApi
				? new Function('dv', 'current', `return ${filesFromDVQuery}`)(
						dvApi,
						dvApi.page(targetFilePath)
					)
				: void 0
		const matchingEdges = orientedEdges(direction, edges, node)
		const linkNodes = matchingEdges
			.filter((edge) => !edgeLabels || edgeLabels.length === 0 || edgeLabels.includes(edge.label))
			.filter(
				(edge) =>
					!edgeColors ||
					edgeColors.length === 0 ||
			...
			.filter(
				(edge) =>
					!edgeFromSides || edgeFromSides.length === 0 || edgeFromSides.includes(edge.fromSide)
			)
			.filter(
				(edge) => !edgeToSides || edgeToSides.length === 0 || edgeToSides.includes(edge.toSide)
			)
			.map((edge) => targetNode(direction, edge, nodes, node))
			.filter((node2) => !!node2 && node2.type === 'file')
			.filter(
				(node2) =>
					!nodeColors ||
					nodeColors.length === 0 ||
					(!node2.color && nodeColors.includes('0')) ||
					nodeColors.includes(node2.color)
			)
			.filter((node2) => {
				return (
					matchingFiles === void 0 || matchingFiles.map((f2) => f2.file.path).includes(node2.file)
				)
			})
		const uniqueLinkNodes = [...new Map(linkNodes.map((link) => [link.file, link])).values()]
		cumulativeSet.set(field2.name, [
			...(cumulativeSet.get(field2.name) || []),
			...uniqueLinkNodes.filter((link) => {
				var _a2
				return !((_a2 = cumulativeSet.get(field2.name)) == null
					? void 0
					: _a2.map((link2) => link2.id).includes(link.id))
			})
		])
	}
	...
	const resolveFieldGroupsForNode = (field2, canvasGroups, node, cumulatedGroupsFields) => {
		...
		cumulatedGroupsFields.set(field2.name, [
			...(cumulatedGroupsFields.get(field2.name) || []),
			...groupNodes.filter((group) => {
				var _a2
				return !((_a2 = cumulatedGroupsFields.get(field2.name)) == null
					? void 0
					: _a2.map((group2) => group2.id).includes(group.id))
			})
		])
	}
	canvases.forEach(async (canvas) => {
			...
			if (node.type === 'file' && dvApi) {
				...
				const groupsFields =
					fileFields2 == null
						...
						: fileFields2.filter(
								(field2) =>
									field2.type === 'CanvasGroup' && field2.options.canvasPath === canvas.path
							)
				const groupsLinksFields =
					fileFields2 == null
						? void 0
						: fileFields2.filter(
								(field2) =>
									field2.type === 'CanvasGroupLink' && field2.options.canvasPath === canvas.path
							)
				linksFields == null
					? void 0
					: linksFields.forEach((field2) => {
							resolveFieldLinksForNode(
								field2,
								targetFilePath,
								edges,
								nodes,
				...
				groupsFields == null
					? void 0
					: groupsFields.forEach((field2) => {
							resolveFieldGroupsForNode(field2, canvasGroups, node, cumulatedGroupsFields)
						})
				groupsLinksFields == null
					? void 0
					: groupsLinksFields.forEach((field2) => {
							const groupNodes = filterGroupsForField(field2, canvasGroups, node)
							...
							if (groupNodes.length === 0) {
								cumulatedGroupsLinksFields.set(field2.name, [
		...
		currentFiles.forEach(
			async (
				{ cumulatedLinksFields, cumulatedGroupsFields, cumulatedGroupsLinksFields },
				filePath
			) => {
				const file = plugin.app.vault.getAbstractFileByPath(filePath)
				if (file && file instanceof import_obsidian71.TFile) {
					...
					cumulatedLinksFields.forEach((linkNodes, name) => {
						...
						const values = linkNodes.map((node) =>
							buildMarkDownLink(plugin, file, node.file, node.subpath)
						)
						if (field2)
							payload.push({
								indexedPath: field2.id,
								payload: { value: values ? [...new Set(values)].join(',') : '' }
							})
					})
					cumulatedGroupsFields.forEach((groupNodes, name) => {
						...
						if (field2)
							payload.push({
								indexedPath: field2.id,
								payload: { value: values ? [...new Set(values.filter((v) => !!v))].join(',') : '' }
							})
					})
		...
		previousFilesPaths
			.filter((f2) => !currentFilesPaths.includes(f2))
			.forEach(async (filePath) => {
				...
				if (targetFile && targetFile instanceof import_obsidian71.TFile) {
					const payload = []
					const canvasFields =
						(_a2 = f.filesFields.get(filePath)) == null
							? void 0
							: _a2.filter(
									(field2) => field2.type === 'Canvas' && field2.options.canvasPath === canvas.path
								)
					canvasFields == null
						? void 0
						: canvasFields.forEach((field2) => {
								payload.push({ indexedPath: field2.id, payload: { value: '' } })
							})
					const canvasGroupFields =
						(_b = f.filesFields.get(filePath)) == null
							? void 0
							: _b.filter(
									(field2) =>
					...
					canvasGroupFields == null
						? void 0
						: canvasGroupFields.forEach((field2) => {
								payload.push({ indexedPath: field2.id, payload: { value: '' } })
							})
					const canvasGroupLinksFields =
						(_c = f.filesFields.get(filePath)) == null
							? void 0
							: _c.filter(
									(field2) =>
										field2.type === 'CanvasGroupLink' && field2.options.canvasPath === canvas.path
								)
					canvasGroupLinksFields == null
						? void 0
						: canvasGroupLinksFields.forEach((field2) => {
								payload.push({ indexedPath: field2.id, payload: { value: '' } })
							})
...
async function updateCanvasAfterFileClass(plugin, files = []) {
	...
	for (const file of files) {
		...
		if (index.classFilesPath && !file.path.startsWith(index.classFilesPath)) {
			...
			const canvasFields =
				(fileClassName &&
					((_b = index.fileClassesFields.get(fileClassName)) == null
						? void 0
						: _b.filter((field2) => field2.type === 'Canvas'))) ||
				[]
			await Promise.all(
				canvasFields.map(async (field2) => {
				...
				legacyGenuineKeys(dvFile).forEach((key) => {
					if (key !== 'file' && !Object.keys(dvFile.file.frontmatter || {}).includes(key)) {
						...
						try {
							...
							const fieldType = capitalize(type)
							const attr = new FileClassAttribute(
								plugin,
								key,
								this.name,
								fieldType,
	...
	async migrate(fileClass) {
		...
		if (!fileClass.getMajorVersion() || fileClass.getMajorVersion() < 2) {
			...
			await this.plugin.app.fileManager.processFrontMatter(file, async (fm) => {
				...
				fm.fields = [
					...fm.fields.filter((f) => !fields.map((_f) => _f.id).includes(f.id)),
					...fields
				]
				fm.version = '2.0'
			})
...
var FieldIndexBuilder = class extends import_obsidian72.Component {
	constructor(plugin) {
		...
		this.dvReady = () => {
			var _a, _b
			return (
				((_a = this.dv) == null ? void 0 : _a._loaded) &&
				!!((_b = this.plugin.app.plugins.plugins.dataview) == null ? void 0 : _b.index.initialized)
			)
		}
	}
	init() {
...
var FieldIndex = class extends FieldIndexBuilder {
	...
	async onload() {
		...
		this.registerEvent(
			this.plugin.app.metadataCache.on('resolved', async () => {
				if (this.plugin.app.metadataCache.inProgressTaskCount === 0 && this.plugin.launched) {
					if (
						this.changedFiles.every(
							(file) => this.classFilesPath && file.path.startsWith(this.classFilesPath)
						)
					) {
		...
		this.registerEvent(
			this.plugin.app.metadataCache.on('dataview:metadata-change', async (op, file) => {
				if (
					file.stat.mtime > this.launchTime &&
					op === 'update' &&
					this.dvReady() &&
					this.settings.isAutoCalculationEnabled
				) {
					...
					if (
						[...this.dVRelatedFieldsToUpdate.values()].every((item) => item.status === 'processed')
					)
						this.dVRelatedFieldsToUpdate = /* @__PURE__ */ new Map()
				}
			})
	...
	indexableFiles() {
		return this.plugin.app.vault
			.getMarkdownFiles()
			.filter((f) => !this.classFilesPath || !f.path.startsWith(this.classFilesPath))
			.filter(
				(f) => !this.settings.fileIndexingExcludedFolders.some((path) => f.path.startsWith(path))
			)
			.filter(
				(f) =>
					!this.settings.fileIndexingExcludedExtensions.some((extension) =>
						f.path.endsWith(extension)
					)
			)
			.filter(
				(f) =>
					!this.settings.fileIndexingExcludedRegex.some((regexStr) => {
						try {
	...
	pushPayloadToUpdate(filePath, fieldsPayloadToUpdate) {
		const currentFieldsPayloadToUpdate = this.dVRelatedFieldsToUpdate.get(filePath) || {
			status: 'toProcess',
			fieldsPayload: []
		}
		for (const fieldPayload of fieldsPayloadToUpdate) {
			...
			const currentField =
				currentFieldsPayloadToUpdate == null
					? void 0
					: currentFieldsPayloadToUpdate.fieldsPayload.find(
							(item) => item.indexedPath === indexedPath
						)
	...
	async applyUpdates() {
		await Promise.all(
			[...this.dVRelatedFieldsToUpdate.keys()].map(async (filePath) => {
				var _a
				const fieldsPayload =
					(_a = this.dVRelatedFieldsToUpdate.get(filePath)) == null ? void 0 : _a.fieldsPayload
				if (fieldsPayload) {
					await postValues(this.plugin, fieldsPayload, filePath)
					fieldsPayload.forEach((fieldPayload) => {
						var _a2
						const field2 =
							(_a2 = this.filesFields.get(filePath)) == null
								? void 0
								: _a2.find((_f) => _f.isRoot() && _f.id === fieldPayload.indexedPath)
						if (field2 && field2.type === 'Lookup')
							this.fileLookupFieldsStatus.set(
								`${filePath}__${field2.name}`,
								'upToDate' /* upToDate */
							)
						if (field2 && field2.type === 'Formula')
							this.fileFormulaFieldsStatus.set(
								`${filePath}__${field2.name}`,
								'upToDate' /* upToDate */
							)
					})
	...
	async getValuesListNotePathValues() {
		await Promise.all(
			[...new Set(this.fileClassesName)].map(async ([fileClassName, fileClass]) => {
				await Promise.all(
					fileClass.attributes.map(async (attr) => {
		...
		await Promise.all(
			this.plugin.presetFields.map(async (setting) => {
			...
			if (fileClassName) {
				const parent =
					(_b =
						...
						: _b.extends
				if (parent) {
					const parentFile = this.plugin.app.vault.getAbstractFileByPath(
						`${this.classFilesPath || ''}${parent}.md`
					)
					if (parentFile) {
	...
	getAncestorsRecursively(fileClassName) {
		...
		if (ancestors && ancestors.length) {
			...
			const lastAncestorParent =
				(_a = this.fileClassesAncestors.get(lastAncestor)) == null ? void 0 : _a[0]
	...
	getFileClasses() {
		...
		;(async () => {
			await Promise.all(
				this.openFileClassManagerAfterIndex.map(async (fileClassName) => {
	...
	resolveFileClassBinding(itemMatchingFileClasses, filesFieldsFromBinding, itemToMatch, cFile) {
		...
		if (fileClass) {
			this.filesFileClasses.set(filePath, [
				.../* @__PURE__ */ new Set([...(this.filesFileClasses.get(filePath) || []), fileClass])
			])
			this.filesFileClassesNames.set(cFile.path, [
				.../* @__PURE__ */ new Set([
					...(this.filesFileClassesNames.get(filePath) || []),
					fileClass.name
				])
			])
			...
			if (fileFileClassesFieldsFromBinding) {
				...
				const filteredCurrentFields =
					(currentFields == null
						? void 0
						: currentFields.filter((field2) => {
								var _a, _b
								return (
									!newFields.map((f) => f.id).includes(field2.id) &&
									!((_b = (_a = fileClass.options) == null ? void 0 : _a.excludes) == null
										? void 0
										: _b.map((attr) => attr.id).includes(field2.id))
								)
							})) || []
	...
	resolveFileClassMatchingTags() {
		...
		this.indexableFiles().forEach((_f) => {
			...
			const cachedTags =
				(_a = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _a.tags
			let fileTags = []
			...
			const filteredTagsFromFile =
				((_b = cache == null ? void 0 : cache.tags) == null
					? void 0
					: _b.filter((_t) => mappedTags.includes(_t.tag)).map((_t) => _t.tag)) || []
	...
	getFilesForItems(items, groups, filesWithGroups, path = '') {
		if (groups.includes(path || '/')) {
			items
				.filter((_i) => _i.type === 'file')
				.forEach((_i) => filesWithGroups.push({ path: _i.path, group: path || '/' }))
		}
	...
	resolveFileClassQueries() {
		...
		this.settings.fileClassQueries.forEach((sfcq) => {
			...
			fcq.getResults(dvApi).forEach((result) => {
				...
				if (fileClass) {
					const f = result.file
					this.filesFileClasses.set(f.path, [
						.../* @__PURE__ */ new Set([...(this.filesFileClasses.get(f.path) || []), fileClass])
					])
					this.filesFileClassesNames.set(f.path, [
						.../* @__PURE__ */ new Set([
							...(this.filesFileClassesNames.get(f.path) || []),
							fileClass.name
						])
					])
				}
	...
	getFilesFieldsFromFileClass() {
		this.indexableFiles().forEach((f) => {
			...
			const fileClassesCache =
				(_b =
					(_a = this.plugin.app.metadataCache.getFileCache(f)) == null ? void 0 : _a.frontmatter) ==
				null
					? void 0
					: _b[this.settings.fileClassAlias]
			if (fileClassesCache) {
							...
							const filteredCurrentFields =
								(currentFields == null
									...
									: currentFields.filter((field2) => {
											var _a2, _b2
											return (
												!newFields.map((f2) => f2.id).includes(field2.id) &&
												!((_b2 = (_a2 = fileClass.options) == null ? void 0 : _a2.excludes) == null
													? void 0
													: _b2.map((attr) => attr.id).includes(field2.id))
											)
										})) || []
							newFields.push(...filteredCurrentFields)
	...
	getFilesFields() {
		const filesToIndex = this.indexableFiles().filter(
			(_f) =>
				!this.changedFiles.length || //forceupdateAll
				this.changedFiles.filter(
					(file) => this.classFilesPath && file.path.startsWith(this.classFilesPath)
				).length || // a fileclass has changed forceupdate all
		...
		filesToIndex.forEach((f) => {
			...
			let fileFields2
			const previousFileFields =
				((_a = this.previousFilesFields.get(f.path)) == null ? void 0 : _a.map((_f) => _f.id)) || []
			if (
				(fileFieldsFromInnerFileClasses == null ? void 0 : fileFieldsFromInnerFileClasses.length) ||
				(fileFieldsFromQuery == null ? void 0 : fileFieldsFromQuery.length) ||
				(fileFieldsFromTag == null ? void 0 : fileFieldsFromTag.length) ||
				(fileFieldsFromPath == null ? void 0 : fileFieldsFromPath.length) ||
				(fileFieldsFromGroup == null ? void 0 : fileFieldsFromGroup.length)
			) {
				fileFields2 = fileFieldsFromInnerFileClasses || []
				fileFields2.push(
					...(fileFieldsFromTag || []).filter(
						(field2) => !fileFields2.map((f2) => f2.id).includes(field2.id)
					)
				)
				fileFields2.push(
					...(fileFieldsFromPath || []).filter(
						(field2) => !fileFields2.map((f2) => f2.id).includes(field2.id)
					)
				)
				fileFields2.push(
					...(fileFieldsFromGroup || []).filter(
						(field2) => !fileFields2.map((f2) => f2.id).includes(field2.id)
					)
				)
				fileFields2.push(
					...(fileFieldsFromQuery || []).filter(
						(field2) => !fileFields2.map((f2) => f2.id).includes(field2.id)
					)
				)
				...
				filesLookupAndFormulasFields.push(
					...(fileFieldsFromTag || []).filter(
						(field2) =>
							!filesLookupAndFormulasFields.map((f2) => f2.id).includes(field2.id) &&
							this.isLookupOrFormula(field2)
					)
				)
				filesLookupAndFormulasFields.push(
					...(fileFieldsFromPath || []).filter(
				...
				this.filesFields.set(f.path, fileFields2)
				const filesLookupAndFormulasFields = this.fieldsFromGlobalFileClass.filter((f2) =>
					this.isLookupOrFormula(f2)
				)
				if (filesLookupAndFormulasFields.length)
					this.filesLookupsAndFormulasFields.set(
						f.path,
						this.fieldsFromGlobalFileClass.filter((f2) => this.isLookupOrFormula(f2))
					)
				...
				if (filesLookupAndFormulasFields.length)
					this.filesLookupsAndFormulasFields.set(
						f.path,
						fileFields2.filter((f2) => this.isLookupOrFormula(f2))
					)
			}
			if (
				fileFields2.some(
					(f2) => !(previousFileFields == null ? void 0 : previousFileFields.includes(f2.id))
				) ||
				(previousFileFields == null
					? void 0
					: previousFileFields.some((fId) => !fileFields2.map((_f) => _f.id).includes(fId)))
			) {
	...
	getFilesLookupAndFormulaFieldsExists(file) {
		...
		;[...this.filesFields.entries()].forEach(([filePath, fields]) => {
			...
			if (!this.settings.isAutoCalculationEnabled)
				dvFormulaFields.forEach((field2) =>
					this.fileFormulaFieldsStatus.set(
						`${filePath}__${field2.name}`,
	...
	dvQFieldChanged(path) {
		var _a
		let changed = false
		;(_a = this.filesLookupAndFormulaFieldsExists.get(path)) == null
			? void 0
			: _a.forEach((field2) => {
					if (field2.type === 'Lookup') {
						changed =
							changed ||
							this.fileLookupFieldsStatus.get(path + '__' + field2.name) === 'changed' /* changed */
					} else if (field2.type === 'Formula') {
						changed =
							changed ||
							this.fileFormulaFieldsStatus.get(path + '__' + field2.name) ===
								'changed' /* changed */
					}
				})
		return changed
		...
		;(_a = this.filesLookupAndFormulaFieldsExists.get(path)) == null
			...
			: _a.forEach((field2) => {
					if (field2.type === 'Lookup') {
						changed =
							changed ||
							this.fileLookupFieldsStatus.get(path + '__' + field2.name) ===
								'mayHaveChanged' /* mayHaveChanged */
					} else if (field2.type === 'Formula') {
						changed =
							changed ||
		...
		;(_a = this.filesLookupAndFormulaFieldsExists.get(path)) == null
			...
			: _a.forEach((field2) => {
					if (field2.type === 'Lookup') {
						changed =
							changed ||
							this.fileLookupFieldsStatus.get(path + '__' + field2.name) === 'error' /* error */
					} else if (field2.type === 'Formula') {
						changed =
							changed ||
							this.fileFormulaFieldsStatus.get(path + '__' + field2.name) === 'error' /* error */
					}
				})
		return changed
	}
...
var IndexStatus = class extends import_obsidian74.Component {
	...
	onload() {
		...
		this.statusIcon = this.statusBtn.buttonEl.createEl('span', {
...
function fieldModifier(plugin, dv, p, fieldName, attrs = {}) {
	...
	const { indexedPath, field: field2 } = getFullIndexedPathFromDottedPath(
		fieldName,
		plugin.fieldIndex.filesFields.get(file.path)
	)
	...
	if (fieldValue === void 0) {
		...
		addFieldBtn.onclick = async () => {
			if (field2) {
				...
				new chooseSectionModal(plugin, file, (lineNumber, asList, asBlockquote) => {
					const fieldVM = new (FieldValueManager(
						plugin,
						field2,
						file,
						void 0,
		...
		if (field2 && (field2 == null ? void 0 : field2.type)) {
			...
			if (field2.type === 'ObjectList' && !isNaN(parseInt(fieldSegments.last()))) {
				...
				const editBtn = fieldContainer.createEl('button')
				const displayValue30 = dv.el(
					'span',
					displayItem(fieldVM, fieldValue, parseInt(fieldSegments.last())) || '',
					attrs
				)
				...
				editBtn.onclick = async () => {
					const upperObjectListEF = await Note.getExistingFieldForIndexedPath(
						plugin,
						file,
						indexedPath.replace(/\[\d+\]$/, '')
					)
					const item = ((await (upperObjectListEF == null
						? void 0
						: upperObjectListEF.getChildrenFields(fieldVM.plugin, fieldVM.target))) || [])[index]
...
function getFullIndexedPathFromDottedPath(dottedPath, fileFields2) {
	...
	for (const field2 of dottedFields) {
		const f =
			fileFields2 == null ? void 0 : fileFields2.find((x) => x.name === field2 && x.path === parent)
		if (f) {
			fields.push(f)
...
var inlineFieldRegex = (attribute) =>
	`(?<inQuote>>*(\\s+)?)?(?<inList>- )?(?<preSpacer>(\\s+)?)?(?<startStyle>[_\\*~\`]*)(?<attribute>${attribute})(?<endStyle>[_\\*~\`]*)(?<beforeSeparatorSpacer>\\s*)::(?<afterSeparatorSpacer>\\s*)`
var LocationWrapper = {
	fullLine: { start: '', end: '' },
	brackets: { start: '[', end: ']' },
	parenthesis: { start: '(', end: ')' }
}
async function postNamedFieldsValues(
	plugin,
	payload,
	fileOrFilePath,
	lineNumber,
	...
	const inFrontmatter = !!(
		lineNumber === -1 ||
		(lineNumber && start2 && end2 && lineNumber >= start2.line && lineNumber <= end2.line)
	)
...
async function postFieldsInline(
	plugin,
	file,
	fieldsToUpdate,
	fieldsToCreate,
	...
	const updateContentWithField = (content, fieldName, payload) => {
		return content.map((line, i) => {
			...
			if (
				...
				const inputArray = payload.value
					? payload.value
							.replace(/(\,\s+)/g, ',')
							.split(',')
							.sort()
				...
				const inSentenceRegexBrackets2 = new RegExp(
					`\\[${inlineFieldRegex(fieldName)}(?<values>[^\\]]+)?\\]`,
					'gu'
				)
	...
	const editor =
		(_a = plugin.app.workspace.getActiveViewOfType(import_obsidian77.MarkdownView)) == null
			? void 0
			: _a.editor
	if (editor) {
...
var MetadataMenuApi = class {
	...
	getValuesForIndexedPath() {
		return async (fileOrFilePath, indexedPath) =>
			getValuesForIndexedPath(this.plugin, fileOrFilePath, indexedPath)
	}
	fieldModifier() {
	...
	postValues() {
		return async (fileOrFilePath, payload, lineNumber, asList, asBlockquote) =>
			postValues(this.plugin, payload, fileOrFilePath, lineNumber, asList, asBlockquote)
	}
	postNamedFieldsValues() {
		return async (fileOrFilePath, payload, lineNumber, asList, asBlockquote) =>
			postNamedFieldsValues(this.plugin, payload, fileOrFilePath, lineNumber, asList, asBlockquote)
	}
}
...
var migrateSettings = async (plugin) => {
	if (plugin.settings.settingsVersion === void 0) await migrateSettingsV1toV2(plugin)
...
var ValueSuggest = class extends import_obsidian78.EditorSuggest {
	constructor(plugin) {
		...
		this.filterOption = (firstValues, lastValue, option) => {
			return (
				!firstValues ||
				(!(firstValues == null ? void 0 : firstValues.contains(encodeLink(option))) &&
					(!lastValue || (!!lastValue && encodeLink(option).includes(lastValue))))
			)
		}
		this.plugin = plugin
	...
	onTrigger(cursor, editor, file) {
		...
		if ((file == null ? void 0 : file.extension) !== 'md') return null
		if ((_a = editor.editorComponent) == null ? void 0 : _a.table) return null
		...
		if (this.inFrontmatter) {
			...
			if (!regex.test(fullLine) && !fullLine.startsWith(listPrefix)) {
				...
				if (!['', ' '].includes(line.slice(separatorPos + 1, separatorPos + 2))) {
					editor.replaceRange(
						' ',
						{ line: cursor.line, ch: separatorPos + 1 },
						{ line: cursor.line, ch: separatorPos + 1 }
					)
				}
			}
	...
	getAlias(tFile) {
		...
		if (dvApi && ((_b = this.field) == null ? void 0 : _b.options.customRendering)) {
			alias = new Function('page', `return ${this.field.options.customRendering}`)(
				dvApi.page(tFile.path)
			)
		}
		return alias
	}
	...
	async getValueSuggestions(context) {
		...
		if (!this.inFrontmatter) {
			...
			const activeLineField = lineFields.find(
				(lineField) => lineField.index <= position && lineField.index + lineField.length >= position
			)
			if (activeLineField) {
			...
			const regexResult = context.editor
				.getRange({ line: fieldLine, ch: 0 }, context.end)
				.match(fieldRegex)
			...
			let i = 1
			while (
				context.editor.getLine(fieldLine + i).startsWith(listPrefix) &&
				fieldLine + i <= context.editor.lastLine()
			) {
				const regexResult2 = context.editor.getLine(fieldLine + i).match(listItemRegex)
				if (regexResult2 == null ? void 0 : regexResult2.groups)
					(_c = matchField.values) == null
						? void 0
						: _c.push(...splitValues(regexResult2.groups.values))
				i += 1
			}
		}
		if (matchField.attribute) {
			...
			this.field =
				(_d = this.plugin.fieldIndex.filesFields.get(context.file.path)) == null
					? void 0
					: _d.find((f) => f.name === fieldName)
			...
			if (this.field && ['Cycle', 'Multi', 'Select'].includes(this.field.type)) {
				...
				const sortingMethod =
					new Function('a', 'b', `return ${this.field.options.customSorting}`) ||
					function (a, b) {
						return a.basename < b.basename ? -1 : 1
					}
				const fieldVM = fieldValueManager(
				...
				if (lastValue) {
					const results = files
						.filter((f) => {
							var _a2
							return (
								f.basename.toLowerCase().includes(lastValue.toLowerCase()) ||
								((_a2 = this.getAlias(f)) == null
									? void 0
									: _a2.toLowerCase().includes(lastValue.toLowerCase()))
							)
						})
						.map((f) => {
							return Object({
								attr: fieldName,
								value: buildMarkDownLink(
									this.plugin,
									context.file,
									f.basename,
									void 0,
					...
					return files.map((f) => {
						...
						return Object({
							attr: fieldName,
							value: buildMarkDownLink(this.plugin, context.file, f.basename, void 0, alias)
						})
					})
				}
			} else {
	...
	renderSuggestion(suggestion, el) {
		...
		const [rawValue, alias] = `${suggestion.value}`
			.replace(/^\[\[/, '')
			.replace(/\]\]$/, '')
			.split('|')
		...
		if (dvApi && this.field && this.field.options.customRendering && targetFile) {
			if (alias) {
				...
				el.setText(
					new Function('page', `return ${this.field.options.customRendering}`)(
						dvApi.page(targetFile.path)
					)
				)
			}
		} else {
	...
	getFieldLines(editor, fieldName) {
		...
		while (
			!editor.getLine(beginFieldLineNumber).startsWith(`${fieldName}:`) &&
			beginFieldLineNumber > 0
		)
			beginFieldLineNumber = beginFieldLineNumber - 1
		while (
			!editor.getLine(endFieldLineNumber + 1).includes(':') &&
	...
	async selectSuggestion(suggestion, event) {
		...
		if (!(file instanceof import_obsidian78.TFile)) return
		...
		if (this.inFrontmatter && fieldName === 'tags') {
			...
			try {
				...
				if (this.plugin.settings.frontmatterListDisplay === 'asList' /* asList */) {
					...
					const options2 = Object.keys(this.plugin.app.metadataCache.getTags()).map((t) =>
						t.replace(/^#/, '')
					)
					if (typeof pastValues == 'string') {
						valuesArray = [
							...new Set(
								[clean(pastValues), ...valuesArray].filter((item) => options2.includes(item))
							)
						]
  ...
  - ${clean(value)}`
					...
					if (!pastValues) {
						...
						if (!pastValues.contains(',')) {
							...
							const pastValuesArray = pastValues
								.split(',')
								.map((o) => o.trim())
								.slice(0, -1)
							newField =
								attr + ': [' + pastValuesArray.join(', ') + ', ' + clean(suggestion.value) + ']'
						}
					} else if (Array.isArray(pastValues)) {
						if (activeLine.endsWith(',]') || activeLine.endsWith(', ]')) {
							...
							newField =
								attr +
								': [' +
								[...pastValues.slice(0, -1), clean(suggestion.value)].join(', ') +
								']'
						}
					} else {
				...
				if (this.plugin.settings.frontmatterListDisplay === 'asList' /* asList */) {
					...
					while (
						!editor.getLine(endFieldLineNumber2 + 1).includes(':') &&
						!(editor.getLine(endFieldLineNumber2 + 1) === '---')
					) {
						editor.getLine(endFieldLineNumber2)
						endFieldLineNumber2 = endFieldLineNumber2 + 1
					}
					editor.setCursor({
						line: endFieldLineNumber2,
						ch: editor.getLine(endFieldLineNumber2).length
					})
				}
			} catch (error) {
			...
			const note = await Note.buildNote(this.plugin, file)
			const eF = note.existingFields.find(
				(eF2) => eF2.field.isRoot() && eF2.field.name === fieldName
			)
			if (!(eF == null ? void 0 : eF.indexedPath)) return
			const currentValue = Array.isArray(eF.value) ? eF.value : !eF.value ? [] : [eF.value]
			if (this.inFrontmatter) {
				const { beginFieldLineNumber, endFieldLineNumber } = this.getFieldLines(editor, fieldName)
				const renderedValue = note.renderFieldValue(
					eF.field,
					[...currentValue, suggestion.value].join(','),
					this.inFrontmatter ? 'yaml' : 'inline'
				)
				const newField =
					Array.isArray(renderedValue) &&
					((_c = this.field) == null ? void 0 : _c.getDisplay()) === 'asArray' /* asArray */
...
${renderedValue.map((v) => '  - ' + v).join('\n')}`
				...
				if (
					!((((_d = this.field) == null ? void 0 : _d.getDisplay()) === 'asList') /* asList */) &&
					!(
						(
							fieldName === 'tags' && this.plugin.settings.frontmatterListDisplay === 'asList'
						) /* asList */
					)
				) {
				...
				editor.setCursor({
					...
					ch: nextBracketPos - currentValueLength + suggestion.value.length
				})
			}
		}
		this.didSelect = true
...
var updateProps = async (plugin, view, file) => {
	...
	view.metadataEditor.rendered.forEach((item) => {
		...
		const pseudoField = {
			id:
				key === plugin.settings.fileClassAlias
					? `fileclass-field-${plugin.settings.fileClassAlias}`
					: (_b =
								(_a = plugin.fieldIndex.filesFields.get(file.path)) == null
									? void 0
									: _a.find((_f) => _f.isRoot() && _f.name === key)) == null
						? void 0
						: _b.id,
			type:
				key === plugin.settings.fileClassAlias
					? 'Select'
					: (_d =
								(_c = plugin.fieldIndex.filesFields.get(file.path)) == null
									? void 0
									: _c.find((_f) => _f.isRoot() && _f.name === key)) == null
						? void 0
						: _d.type
		}
	...
	const actionContainer =
		view.metadataEditor.contentEl.find('.action-container') ||
		view.metadataEditor.contentEl.createDiv({ cls: 'action-container' })
	actionContainer.replaceChildren()
	actionContainer.appendChild(view.metadataEditor.addPropertyButtonEl)
	const fileClassButtonsContainer =
		view.metadataEditor.contentEl.find('.fileclass-btn-container') ||
		view.metadataEditor.contentEl.createDiv({ cls: 'fileclass-btn-container' })
	...
	fileClasses == null
		...
		: fileClasses.forEach((fileClass) => {
				...
				addFieldButton.onClick(() =>
					new InsertFieldSuggestModal(plugin, file, -1, false, false).open()
				)
			})
...
async function updatePropertiesSection(plugin) {
	...
	for (const leaf of leaves) {
		...
		if (
			!(view instanceof import_obsidian79.MarkdownView) ||
			!(view.file instanceof import_obsidian79.TFile) ||
			view.file === void 0
		)
			return
		const file = view.file
	...
	if (currentView && currentView.file) {
		...
		if (focusedElement instanceof HTMLElement) {
			...
			const field2 =
				key &&
				((_a = plugin.fieldIndex.filesFields.get(currentView.file.path)) == null
					? void 0
					: _a.find((_f) => _f.isRoot() && _f.name === key))
			if (field2) {
				const eF = note.getExistingFieldForIndexedPath(field2.id)
				;(_b = focusedElement.find('[class^=metadata-input]')) == null
					? void 0
					: _b.setText((eF == null ? void 0 : eF.value) || '')
			} else if (key === plugin.settings.fileClassAlias) {
				const eF = note.getExistingFieldForIndexedPath(
					`fileclass-field-${plugin.settings.fileClassAlias}`
				)
				;(_c = focusedElement.find('[class^=metadata-input]')) == null
					? void 0
					: _c.setText((eF == null ? void 0 : eF.value) || '')
			}
		}
	}
}
function getPropView(plugin) {
	...
	plugin.app.workspace.iterateAllLeaves((l) => {
		...
		if (
			!propView &&
			l.view.file &&
			((_a = l.view.plugin) == null ? void 0 : _a.id) === 'properties'
		) {
			propView = l.view
		}
	})
...
async function updatePropertiesPane2(plugin) {
	...
	if (
		propView &&
		propView.file.path == ((_a = plugin.app.workspace.getActiveFile()) == null ? void 0 : _a.path)
	) {
...
var FileClassFolderButton = class extends import_obsidian80.Component {
	...
	addButton() {
		var _a, _b, _c
		const fCFolderPath =
			(_a = this.plugin.settings.classFilesPath) == null ? void 0 : _a.replace(/\/$/, '')
		const explorerView =
			(_c =
				(_b = this.plugin.app.workspace.getLeavesOfType('file-explorer')) == null
					? void 0
					: _b[0]) == null
				? void 0
				: _c.view
	...
	static removeBtn(plugin) {
		var _a, _b, _c
		const fCFolderPath =
			(_a = plugin.settings.classFilesPath) == null ? void 0 : _a.replace(/\/$/, '')
		const explorerView =
			(_c =
				(_b = plugin.app.workspace.getLeavesOfType('file-explorer')) == null ? void 0 : _b[0]) ==
			null
				? void 0
				: _c.view
		if (!explorerView || !fCFolderPath) return
...
// src/db/DatabaseManager.ts
var INDEXES = {
	fileClassView: [{ name: 'fileClassView', fields: 'fileClassName', unique: true }]
}
		...
		request.onupgradeneeded = () => {
			...
			Object.keys(INDEXES).forEach((storeName) => {
				...
				indexes.forEach((index) =>
					store.createIndex(index.name, index.fields, { unique: index.unique })
				)
			})
		}
	}
...
var FileClassCodeBlockView = class {
	...
	getViewConfig() {
		...
		const partialViewConfig =
			((_a = options2.savedViews) == null
				? void 0
				: _a.find((view) => view.name === this.selectedView)) || defaultConfig
		return {
		...
		try {
			...
			if (this.fileClass) {
				this.itemsPerPage =
					...
					this.fileClass.options.limit ||
					this.plugin.settings.tableViewMaxRecords
				this.startAtItem = content['start'] || 0
				this.showAddField = content['showAddField'] === true || false
				this.fileClassCodeBlockView = new FileClassCodeBlockView(
					this,
					this.tableId,
					this.fileClass,
					paginationContainer,
...
var MetadataMenu = class extends import_obsidian85.Plugin {
	...
	async onload() {
		...
		if (
			!this.app.plugins.enabledPlugins.has('dataview') || //@ts-ignore
...
------------------------------------------`,
		...
		this.registerEvent(
			this.app.vault.on('create', (file) => {
				...
				if (
					file instanceof import_obsidian85.TFile &&
					file.extension === 'md' &&
					this.settings.chooseFileClassAtFileCreation
				) {
